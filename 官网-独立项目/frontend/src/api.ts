...
function fromRelease(item: BackendRelease): Release {
  return {
    ...
    checksumType: item.checksumType || 'sha256',
    manifestUrl: item.manifestUrl || '',
    target: item.target || '',
    channel: item.channel || 'stable',
    platform: item.platform || 'linux-docker-x64',
    build: item.build || '',
    packageSize: item.packageSize || 0,
    minSupportedVersion: item.minSupportedVersion || '',
    forceUpdate: Boolean(item.forceUpdate),
    dockerImageTags: item.dockerImageTags || [],
    releaseNotesMarkdown: item.releaseNotesMarkdown || '',
    status: item.status || 'published',
    publishedAt: item.publishedAt || '',
    updatedAt: item.updatedAt || '',
