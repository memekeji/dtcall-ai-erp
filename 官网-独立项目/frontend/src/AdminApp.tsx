...
function ReleasePanel({ items, update }: { items: Release[]; update: (items: Release[]) => void }) {
  return <PanelIntro title="DT 企业管理系统版本发布" text="这里维护的是客户部署的 DT 企业管理系统升级包、版本说明、目标环境与校验信息，不是官网内容更新。"><EditableList items={items} update={update} emptyText="暂无版本发布，点击新增创建客户部署升级包信息。" create={() => ({ id: Date.now(), version: 'v1.0.0', title: '新版本发布', summary: '', packageUrl: '', checksum: '', target: 'Ubuntu 22.04 x86_64', status: 'draft', publishedAt: new Date().toISOString(), updatedAt: new Date().toISOString(), highlights: [] })} render={(item, index, setItem) => <>
