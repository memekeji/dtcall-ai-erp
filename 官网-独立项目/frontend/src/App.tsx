  ...
  return (
    <div className="app-root">
      ...
      <Routes>
        ...
        <Route path="/resources" element={<GenericPage title={data.pages.resources?.title || '资源中心'} blocks={data.pages.resources?.blocks || data.home.blocks} items={data.resources.resources} faqs={data.resources.faqs} />} />
        ...
        <Route path="/contact" element={<GenericPage title={data.pages.contact?.title || '联系我们'} blocks={data.pages.contact?.blocks || data.home.blocks} faqs={data.resources.faqs} />} />
    ...
    <header className="site-header">
      ...
      <nav className="site-nav" aria-label="官网主导航">
        {data.navigation.filter((item) => item.visible).map((item) => (
          <a key={item.path} href={item.path === '/' ? '/' : item.path.startsWith('/') ? item.path : `#${item.path}`} target={item.target || '_self'} rel={item.target === '_blank' ? 'noreferrer' : undefined}>{item.label}</a>
        ))}
      </nav>
      <div className="header-actions">
        <a className="phone-link" href={`tel:${data.site.phone}`}>{data.site.phone}</a>
        <a className="admin-link" href="/contact">预约演示</a>
      </div>
    </header>
  )
}
