...
class WorkflowService:
        ...
        try:
            ...
            workflow = AIWorkflow.objects.create(
                ...
                is_public=(
                    workflow_data.get(
                        'is_public',
                        False) if workflow_data else False),
        ...
        try:
            ...
            for node_data in nodes_data:
                ...
                if node_id and node_id in existing_node_ids:
                    ...
                    existing_node.name = node_data.get(
                        'name', existing_node.name)
                    existing_node.position_x = node_data.get(
                        'x', node_data.get('position_x', 0))
                    existing_node.position_y = node_data.get(
                        'y', node_data.get('position_y', 0))
                    existing_node.config = node_data.get(
                        'config', existing_node.config)
                    existing_node.is_active = node_data.get(
                        'is_active', existing_node.is_active)
                    existing_node.save()
                    node_ids_to_keep.add(str(existing_node.id))
                else:
                    node_type = node_data.get(
                        'type') or node_data.get('node_type')
                    if not node_type:
                        logger.warning(f'节点缺少类型信息: {node_data}')
                        continue

                    new_node = WorkflowNode.objects.create(
                        workflow=workflow, node_type=node_type, name=node_data.get(
                            'name', f"节点 {node_type}"), position_x=node_data.get(
                            'x', node_data.get(
                                'position_x', 0)), position_y=node_data.get(
                            'y', node_data.get(
                                'position_y', 0)), config=node_data.get(
                            'config', {}), is_active=node_data.get(
                                'is_active', True))
            ...
            if nodes_to_delete:
                WorkflowNode.objects.filter(
                    id__in=list(nodes_to_delete)).delete()
            ...
            for conn_data in connections_data:
                ...
                WorkflowConnection.objects.create(
                    ...
                    source_handle=conn_data.get(
                        'sourcePort') or conn_data.get('source_handle'),
                    target_handle=conn_data.get(
                        'targetPort') or conn_data.get('target_handle'),
    ...
    def get_workflow(self, workflow_id):
        ...
        try:
            ...
            data_access_configs = list(
                workflow.data_access_configs.all().values(
                    'id',
                    'access_type',
                    'resource_name',
                    'resource_identifier',
            ...
            return {
                ...
                'collaborators': list(
                    workflow.collaborators.values_list(
                        'id',
                        flat=True)),
                'nodes': nodes,
    ...
    def _execute_workflow_async(self, execution_id):
        ...
        """
        try:
            execution = AIWorkflowExecution.objects.select_related(
                'workflow').get(id=execution_id)
            ...
            # 查找开始节点 - 支持多种开始节点类型或配置标记
            start_node = next(
                (node for node in nodes.values() if node.node_type == 'start' or (
                    node.node_type == 'data_input' and (node.name == '开始' or node.config.get('is_start_node') is True))), None)
            if not start_node:
                # 尝试找没有输入连接的 data_input 节点
                for node in nodes.values():
                    if node.node_type == 'data_input':
                        has_incoming = any(conn.target_node_id == node.id for conn in connections)
                        if not has_incoming:
                            start_node = node
                            break
                if not start_node:
            ...
            self._execute_node_recursive(
                start_node, execution, execution_graph, context, nodes)
    ...
    def _execute_node_recursive(
        ...
        """
        # 执行当前节点
        result, has_error = self._execute_node(
            node, execution, execution_graph, context, nodes)
            ...
            if next_node_id in nodes:
                ...
                if next_node.node_type == 'condition':
                    self._execute_condition_node(
                        next_node, execution, execution_graph, context, nodes, result)
                else:
                    self._execute_node_recursive(
                        next_node, execution, execution_graph, context, nodes)
    ...
    def _execute_condition_node(
        ...
        condition_met = self._evaluate_condition(
            context_value, condition_operator, condition_value)
        ...
        if processor:
            try:
                ...
                if not validation_result.get('valid', False):
                    logger.warning(
                        f"节点配置验证失败: {validation_result.get('errors', [])}")
    ...
    def _execute_data_transformation(self, config, context):
        ...
        try:
            # 获取配置参数
            transformation_type = config.get(
            ...
            if transformation_type == 'json_to_csv':
                ...
                if isinstance(data, list) and len(data) > 0:
                    ...
                    writer = csv.DictWriter(
                        output_csv, fieldnames=sorted(fieldnames))
                    writer.writeheader()
                    for item in data:
    ...
    def _execute_data_output(self, config, context):
        ...
        try:
            ...
            if output_type == 'variable' and output_variable:
                ...
                # 处理JSON数据
                if headers.get(
                        'Content-Type') == 'application/json' and isinstance(data, str):
                    try:
                ...
                response = requests.request(
                    method, url, data=data, headers=headers, timeout=30)
                output['api_response'] = response.text
                ...
                for key, value in context.items():
                    table_name = table_name.replace(
                        f'{{{{{key}}}}}', str(value))
                    data_to_insert = data_to_insert.replace(
                        f'{{{{{key}}}}}', str(value))
                ...
                if notification_type == 'webhook':
                    ...
                    response = requests.post(
                        webhook_url, json={'message': message})
    ...
    def _execute_multi_condition(self, config, context):
        ...
        try:
            ...
            evaluation_mode = config.get(
                'evaluation_mode',
                'first_match')  # first_match, all_matches
    ...
    def _execute_ai_classification(self, config, context):
        ...
        try:
            ...
            try:
                ...
                output['predicted_class'] = classification_data.get(
                    'class', '')
                output['confidence'] = classification_data.get(
                    'confidence', 0.0)
        ...
        try:
            ...
            try:
                ...
                if extraction_type == 'sentiment':
                    ...
                    output['confidence'] = extraction_data.get(
                        'confidence', 0.0)
                elif extraction_type == 'keywords':
    ...
    def get_workflow_executions(self, workflow_id, **filters):
        ...
        Returns:
            QuerySet: 执行实例查询集
        """
        return AIWorkflowExecution.objects.filter(
            workflow_id=workflow_id, **filters).order_by('-created_at')
    ...
    def save_workflow_design(self, workflow_id, nodes_data, connections_data):
        ...
        try:
            ...
            # 更新工作流节点和连接
            workflow = self.update_workflow_nodes(
                workflow_id, nodes_data, connections_data)
    ...
    def test_node(self, node_type, node_config, input_data, user):
        ...
        try:
            ...
            if node_type == 'data_input':
                ...
                result = self._execute_data_transformation(
                    node_config, context)
            elif node_type == 'text_processing':
