...
请确保返回有效的 JSON 格式，不要包含其他内容。"""
            ...
            messages = [
                {'role': 'system', 'content': '你是一个专业的意图分类器，能够准确识别用户的查询意图。请返回严格的 JSON 格式。'},
    ...
    def _extract_intent_from_text(self, response: str, query: str) -> Dict[str, Any]:
        """从文本响应中提取意图"""
        intent = 'AI_CHAT'
        confidence = 0.5
    ...
    def _rule_based_fallback(self, query: str) -> Dict[str, Any]:
        ...
        # 1. 优先检查问候语（最高优先级）
        greetings = ['你好', '您好', 'hi', 'hello', '早上好', '下午好', '晚上好', '早上好呀', '下午好呀', '晚上好呀']
        ...
        # 2. 检查聊天意图
        chat_keywords = ['聊天', '谈谈', '讨论', '说说', '想', '交流', '闲聊', '随便聊', '聊聊', '在吗', '有人吗']
        ...
        # 3. 检查知识库意图
        knowledge_keywords = ['知识库', '知识', '文档', '帮助', '指南', '手册', '教程', '如何', '怎样', '怎么', '步骤', '方法']
        ...
        # 6. 检查查询意图（业务关键词）
        query_keywords = ['查询', '统计', '数量', '列表', '总数', '进度', '多少', '有多少', '汇总', '数据', '报表', '记录', '看看', '显示']
        ...
        # 7. 检查业务实体（隐含查询意图）
        business_keywords = ['客户', '订单', '合同', '项目', '发票', '员工', '部门', '财务', '生产', '销售', '采购', '库存', '产品', '供应商']
        ...
        # 8. 检查时间词（隐含查询意图）
        time_keywords = ['本月', '上月', '今年', '去年', '本周', '上周', '最近', '当前', '这个月', '上个月']
    ...
    def _enhance_result(self, result: Dict[str, Any], query: str) -> Dict[str, Any]:
        """增强结果，提取更多实体信息"""
        query_lower = query.lower()
