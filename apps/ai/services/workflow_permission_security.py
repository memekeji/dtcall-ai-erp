...
class WorkflowPermissionService:
    ...
    def get_user_permissions(
            self, user_id: int, workflow_id: str) -> Dict[str, bool]:
        """获取用户权限"""
        workflow = AIWorkflow.objects.filter(id=workflow_id).first()
        if not workflow:
            return {}
    ...
    def get_workflow_access_list(
            self, workflow_id: str) -> List[Dict[str, Any]]:
        """获取工作流访问列表"""
        from apps.ai.models_enhanced import WorkflowPermission
...
class AuditLogService:
    ...
    def query_logs(
        ...
        logs = AIWorkflowAuditLog.objects.filter(
            query).order_by('-created_at')[:limit]
