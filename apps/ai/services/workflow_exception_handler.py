...
from apps.ai.models import (
    AIWorkflow, WorkflowNode, AIWorkflowExecution, NodeExecution, AILog
)
...
class NodeException(WorkflowException):
    ...
    def __init__(self, message: str, node_id: str = None, context: Dict[str, Any] = None):
...
class CircuitBreaker:
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        return (timezone.now() - self.state.last_failure_time).total_seconds() >= self.recovery_time
...
class ExceptionClassifier:
    ...
    def classify(cls, exception: Exception) -> tuple:
        ...
        # 查找严重程度
        severity = cls._severity_mapping.get(exception_type, ExceptionSeverity.MEDIUM)
    ...
    @classmethod
    def _find_category(cls, exception_type: Type[Exception], 
                      hierarchy: Dict, current_category: ExceptionCategory = None) -> ExceptionCategory:
        """递归查找异常分类"""
                ...
                if isinstance(mapping, dict):
                    ...
                    if 'subclasses' in mapping:
                        return cls._find_category(
                            exception_type, 
                            mapping['subclasses'],
                            current_category or mapping.get('category', ExceptionCategory.SYSTEM)
                        )
                else:
                    return mapping
...
class ExceptionHandler:
    ...
    def should_retry(self, exception_info: ExceptionInfo) -> bool:
        ...
        # 根据异常类型判断
        if exception_info.category in [ExceptionCategory.NETWORK, ExceptionCategory.SYSTEM]:
    ...
    def get_recovery_strategy(self, exception_info: ExceptionInfo) -> RecoveryStrategy:
        """获取恢复策略"""
        # 根据异常类型和严重程度选择恢复策略
...
class NodeExceptionHandler:
    ...
    async def execute_node(self, node: WorkflowNode, 
        ...
        try:
            ...
            # 获取恢复策略
            strategy = self.global_handler.get_recovery_strategy(exception_info)
    ...
    def _handle_fallback(self, node: WorkflowNode, 
        ...
        # 默认回退值
        default_fallback = node.config.get('error_handling', {}).get('fallback_value')
...
class WorkflowExceptionHandler:
    ...
    async def execute_workflow_with_error_handling(
        ...
        try:
            ...
            nodes = list(workflow.nodes.filter(is_active=True).order_by('position_x', 'position_y'))
            ...
            for node in nodes:
                try:
                    ...
                    execution_result['node_results'][str(node.id)] = node_result
                    ...
                    if node_result.get('status') == 'failed':
                        if not node.config.get('error_handling', {}).get('continue_on_error', False):
                    ...
                    if node.config.get('error_handling', {}).get('continue_on_error', False):
                        continue
                    else:
