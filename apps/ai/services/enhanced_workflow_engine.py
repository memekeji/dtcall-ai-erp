...
class EnhancedWorkflowEngine:
    ...
    async def _execute_parallel(
        ...
        level_map = self._build_node_levels(
            execution_graph, str(start_node.id))
        max_level = max(level_map.values()) if level_map else 0

        for level in range(max_level + 1):
            level_nodes = [
                nid for nid,
                lvl in level_map.items() if lvl == level]
            tasks = []
    ...
    async def _execute_node_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if processor:
                    ...
                    if hasattr(processor, 'execute_async'):
                        result = await asyncio.wait_for(
                            processor.execute_async(
                                node.config, context.variables),
                            timeout=timeout
                        )
                    else:
                        ...
                        result = await asyncio.wait_for(
                            asyncio.to_thread(
                                processor.execute, node.config, context.variables),
                            timeout=timeout
                        )
                else:
                ...
                node_state.execution_time = (
                    node_state.completed_at -
                    node_state.started_at).total_seconds()
                ...
                node_state.retry_count = attempt + 1
                logger.error(
                    f"节点执行失败: {node.name}, 错误: {e}, 重试次数: {attempt + 1}")
    ...
    async def _execute_node_legacy(
            self, node: WorkflowNode, context: Dict[str, Any]) -> Dict[str, Any]:
        """遗留节点执行逻辑"""
        ...
        if node_type == 'start':
            ...
            config.get('condition_type', 'if_else')
    ...
    def _build_execution_graph(self,
                               nodes: Dict[str,
                                           WorkflowNode],
                               connections: List[WorkflowConnection]) -> Dict[str,
                                                                              List[tuple]]:
        """构建执行图"""
        graph = {}
    ...
    def _build_node_levels(
            self, graph: Dict[str, List[tuple]], start_node: str) -> Dict[str, int]:
        """构建节点层级（用于并行执行）"""
        levels = {}
        queue = [(start_node, 0)]
        visited = {start_node}
    ...
    def _find_start_node(
            self, nodes: Dict[str, WorkflowNode]) -> Optional[WorkflowNode]:
        """查找开始节点"""
...
class WorkflowPerformanceMonitor:
    ...
    def record_execution_time(self, workflow_id: str, execution_time: float):
        ...
        self.metrics_cache[workflow_id]['execution_times'].append(
            execution_time)
    ...
    def record_node_time(
        ...
        self.metrics_cache[workflow_id]['node_times'][node_id].append(
            execution_time)
