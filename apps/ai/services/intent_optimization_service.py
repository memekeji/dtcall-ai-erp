...
class IntentOptimizationService:
    ...
    def log_recognition(self, user: User, query: str, recognition_result: Dict[str, Any], 
                       actual_intent: Optional[str] = None, user_feedback: str = '',
        ...
        try:
            log_entry = AIIntentRecognitionLog.objects.create(
                ...
                actual_intent=actual_intent or recognition_result.get('intent', 'UNKNOWN'),
            ...
            logger.info(f"记录意图识别日志：query={query[:50]}, intent={log_entry.recognized_intent}, "
    ...
    def _determine_result(self, recognition_result: Dict[str, Any], is_correct: bool) -> str:
        """确定识别结果状态"""
        confidence = recognition_result.get('confidence', 0.0)
    ...
    def _trigger_optimization(self, log_entry: AIIntentRecognitionLog):
        ...
        try:
            ...
            if incorrect_count >= 5:
                logger.warning(f"意图 {intent_type} 在过去 7 天内有{incorrect_count}次识别错误，触发优化流程")
    ...
    def _optimize_intent_config(self, intent_type: str):
        ...
        try:
            ...
            if new_examples:
                current_examples = intent_config.examples if isinstance(intent_config.examples, list) else []
                ...
                logger.info(f"优化意图配置成功：{intent_type}, 新增{len(new_examples)}个示例")
    ...
    def get_accuracy_statistics(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            logs = AIIntentRecognitionLog.objects.filter(created_at__gte=start_date)
    ...
    def get_low_confidence_queries(self, limit: int = 20) -> List[Dict[str, Any]]:
        """
        获取低置信度查询列表
    ...
    def retrain_intent_model(self, intent_type: Optional[str] = None) -> Dict[str, Any]:
        """
        重新训练意图模型
        ...
        """
        try:
            training_samples = self.get_training_samples(intent_type, limit=500)
            ...
            new_examples = [sample['query_text'] for sample in training_samples]
            ...
            if intent_config:
                ...
                logger.info(f"重新训练意图模型成功：{intent_type}, 使用{len(new_examples)}个样本")
                ...
                for sample in training_samples:
                    ...
                    if sample_intent:
                        sample_intent_config = AIIntentRecognition.objects.filter(
                            intent_type=sample_intent,
                            is_active=True
                        ).first()
                        if sample_intent_config:
                            current_examples = sample_intent_config.examples if isinstance(sample_intent_config.examples, list) else []
                            current_examples.append(sample['query_text'])
                            sample_intent_config.examples = list(set(current_examples))[:100]
