...
class NodeConfigSchema:
    ...
    DATABASE_TABLES = {
        'customer_customer': '客户表', 'contract_contract': '合同表', 'finance_invoice': '发票表',
        'project_project': '项目表', 'auth_user': '用户表',
    }
    ...
    def get_ai_classification_schema(cls) -> Dict[str, FieldSchema]:
        return {
            'model_id': FieldSchema(type='select', required=True, label='AI模型', default='gpt-4'),
            'classification_type': FieldSchema(type='select', required=True, label='分类类型', default='single',
                options=[{'value': 'single', 'label': '单标签'}, {'value': 'multi', 'label': '多标签'}, {'value': 'binary', 'label': '二分类'}]),
            'categories': FieldSchema(type='array', required=True, label='分类选项',
                properties={'value': FieldSchema(type='string', required=True), 'label': FieldSchema(type='string', required=True)}),
            ...
            'temperature': FieldSchema(type='number', required=False, label='Temperature', default=0.3),
        }
    ...
    def get_knowledge_retrieval_schema(cls) -> Dict[str, FieldSchema]:
        return {
            'retrieval_type': FieldSchema(type='select', required=True, label='检索类型', default='vector',
                options=[{'value': 'vector', 'label': '向量检索'}, {'value': 'keyword', 'label': '关键词检索'}, {'value': 'hybrid', 'label': '混合检索'}]),
            'knowledge_base': FieldSchema(type='select', required=True, label='知识库',
                options=[{'value': 'default', 'label': '默认知识库'}, {'value': 'product', 'label': '产品知识库'}, {'value': 'faq', 'label': 'FAQ知识库'}]),
            'query_variable': FieldSchema(type='string', required=True, label='查询变量'),
            'top_k': FieldSchema(type='number', required=False, label='返回数量', default=5),
            'similarity_threshold': FieldSchema(type='number', required=False, label='相似度阈值', default=0.7),
            'output_mode': FieldSchema(type='select', required=False, label='输出模式', default='content',
                options=[{'value': 'content', 'label': '内容'}, {'value': 'full', 'label': '完整信息'}]),
        }
    
    @classmethod
    def get_condition_schema(cls) -> Dict[str, FieldSchema]:
        return {
            'condition_type': FieldSchema(type='select', required=True, label='条件类型', default='if_else',
                options=[{'value': 'if_else', 'label': 'IF-ELSE'}, {'value': 'switch', 'label': 'SWITCH'}, {'value': 'logic', 'label': '逻辑组合'}]),
            'condition_variable': FieldSchema(type='string', required=True, label='条件变量',
                placeholder='例如: data.sentiment'),
            'expressions': FieldSchema(type='array', required=False, label='条件表达式',
                properties={
                    'operator': FieldSchema(type='select', required=True, label='运算符',
                        options=[{'value': '==', 'label': '等于'}, {'value': '>', 'label': '大于'}, {'value': '<', 'label': '小于'}]),
                    'value': FieldSchema(type='string', required=True, label='比较值'),
                    'output': FieldSchema(type='string', required=True, label='输出标签'),
                }),
            'default_output': FieldSchema(type='string', required=False, label='默认输出', default='default'),
        }
    ...
    def get_text_processing_schema(cls) -> Dict[str, FieldSchema]:
        return {
            'processing_type': FieldSchema(type='select', required=True, label='处理类型',
                options=[{'value': 'clean', 'label': '文本清洗'}, {'value': 'transform', 'label': '文本转换'},
                         {'value': 'extract', 'label': '信息提取'}, {'value': 'translate', 'label': '翻译'},
                         {'value': 'summarize', 'label': '摘要生成'}]),
            'input_variable': FieldSchema(type='string', required=True, label='输入变量'),
            'output_variable': FieldSchema(type='string', required=True, label='输出变量'),
            'custom_config': FieldSchema(type='object', required=False, label='自定义配置', default={}),
        }
    
    @classmethod
    def get_data_aggregation_schema(cls) -> Dict[str, FieldSchema]:
        return {
            'group_by': FieldSchema(type='array', required=True, label='分组字段'),
            'aggregations': FieldSchema(type='array', required=True, label='聚合操作',
                properties={
                    'field': FieldSchema(type='string', required=True, label='字段'),
                    'function': FieldSchema(type='select', required=True, label='聚合函数',
                        options=[{'value': 'sum', 'label': '求和'}, {'value': 'avg', 'label': '平均'},
                                 {'value': 'count', 'label': '计数'}, {'value': 'max', 'label': '最大'}]),
                    'alias': FieldSchema(type='string', required=False, label='别名'),
                }),
            'input_variable': FieldSchema(type='string', required=True, label='输入变量'),
            'output_variable': FieldSchema(type='string', required=True, label='输出变量'),
        }
...
class ModuleAssociationService:
    ...
    def get_module_models(cls, module_name: str) -> List[Dict]:
        ...
        for model_name in module.get('models', []):
            try:
                ...
                fields = [{'name': f.name, 'type': f.get_internal_type()} for f in model._meta.fields]
                models.append({'name': model_name, 'verbose_name': model._meta.verbose_name, 'fields': fields})
            except:
                pass
        return models
    ...
    @classmethod
    def generate_model_query_config(cls, module_name: str, model_name: str) -> Dict:
        return {
