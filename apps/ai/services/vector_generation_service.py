...
class VectorGenerationService:
    ...
    def generate_vector_for_knowledge_item(self, knowledge_item_id):
        ...
        try:
            ...
            if knowledge_item.status != 'published':
                logger.info(f"跳过未发布的知识条目 - ID: {knowledge_item_id}, 状态: {knowledge_item.status}")
                return False
            ...
            logger.info(f"成功生成向量 - 知识条目ID: {knowledge_item_id}, 标题: {knowledge_item.title}")
            return True
            ...
            logger.error(f"生成向量失败 - 知识条目ID: {knowledge_item_id_log}, 错误: {str(e)}")
    ...
    def batch_generate_vectors(self, knowledge_item_ids=None):
        ...
        try:
            ...
            logger.info(f"批量生成向量完成 - 成功: {success_count}, 失败: {error_count}, 总数量: {total}")
            return {
    ...
    def _generate_vector(self, knowledge_item):
        ...
        try:
            ...
            logger.info(f"生成向量 - 知识条目ID: {knowledge_item.id}, 原始文本长度: {len(full_text)}")
            ...
            if len(full_text) > max_text_length:
                ...
                logger.info(f"文本过长，已截断 - 知识条目ID: {knowledge_item.id}, 截断后长度: {len(text)}")
            else:
                text = full_text
    ...
    def get_vector_for_knowledge_item(self, knowledge_item_id):
        """获取知识条目的向量"""
        try:
            vector_record = AIKnowledgeVector.objects.get(knowledge_item_id=knowledge_item_id)
            return np.frombuffer(vector_record.vector, dtype=np.float32).tolist()
