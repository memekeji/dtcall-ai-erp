    ...
    def end_debug_session(self, session_id: str):
        ...
        if session_id in self.active_sessions:
            ...
            for node_id in session.breakpoints:
                workflow_debugger.remove_breakpoint(
                    session.workflow_id, node_id)
    ...
    def execute_with_debug(
        ...
        try:
            execution = workflow_engine.create_execution(
                workflow_id, user_id, input_data)
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        for node_id, events in events_by_node.items():
            ...
            node_summary[node_id] = {
                'execution_time': (
                    end_time -
                    start_time).total_seconds() if start_time and end_time else 0,
        ...
        return [
            {
                ...
                'duration': (
                    e.completed_at -
                    e.started_at).total_seconds() if e.started_at and e.completed_at else None,
                'input_data': e.input_data,
                'output_data': e.output_data,
                'error_message': e.error_message} for e in executions]

    def get_workflow_health_status(self, workflow_id: str) -> Dict[str, Any]:
        """获取工作流健康状态"""
        recent_metrics = self.get_workflow_metrics(
            workflow_id, timedelta(hours=24))
...
class PerformanceOptimizationService:
    ...
    def _generate_optimization_suggestions(
            self, node_metrics: Dict[str, Any]) -> List[str]:
        """生成优化建议"""
        suggestions = []
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_cache = {
            k: v for k,
            v in self.execution_cache.items() if (
                datetime.now() -
                v.get(
                    'cached_at',
                    datetime.now())).total_seconds() < self.cache_ttl}
    ...
    def _calculate_hit_rate(self) -> float:
        """计算缓存命中率"""
        hits = sum(
            1 for v in self.execution_cache.values() if v.get(
                'hit', False))
