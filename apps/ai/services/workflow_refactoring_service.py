    ...
    def initialize(cls):
        ...
        cls._layer_mappings = {
            ...
            'ai_capability': NodeLayerMapping(
                ...
                can_call_layers=[NodeLayer.LOGIC_CONTROL.value, NodeLayer.DATA_PROCESSING.value],
            ...
            'logic_control': NodeLayerMapping(
                ...
                node_types=['condition', 'switch', 'loop', 'iterator', 
                           'parallel', 'delay', 'wait'],
                can_call_layers=[NodeLayer.AI_CAPABILITY.value, NodeLayer.DATA_PROCESSING.value],
            ...
            'data_processing': NodeLayerMapping(
                ...
                can_call_layers=[NodeLayer.DATA_IO.value, NodeLayer.INTEGRATION.value],
        ...
        for node in nodes:
            ...
            for conn in source_connections:
                ...
                if caller_layer and callee_layer:
                    if callee_layer.value not in cls.get_allowed_callers(conn.source_node.node_type):
                        errors.append(
...
class DataFlowManager:
    ...
    def initialize(cls):
        ...
        cls._data_flow_rules = {
            ('data_input', 'ai_model'): DataFlowRule(
                ...
                allowed_transformations=['passthrough', 'field_mapping', 'type_cast'],
            ...
            ('ai_model', 'data_output'): DataFlowRule(
                ...
                allowed_transformations=['passthrough', 'field_mapping', 'filter'],
    ...
    @classmethod
    def get_flow_rule(cls, source_type: str, target_type: str) -> Optional[DataFlowRule]:
        """获取数据流转规则"""
    ...
    @classmethod
    def validate_data_flow(cls, source_node: WorkflowNode, target_node: WorkflowNode,
        ...
        """
        flow_rule = cls.get_flow_rule(source_node.node_type, target_node.node_type)
        ...
        # 验证必需字段
        for field_name in flow_rule.validation_schema.get('required_fields', []):
        ...
        # 验证数据类型
        for field_name, expected_type in flow_rule.validation_schema.get('data_types', {}).items():
    ...
    @classmethod
    def transform_data(cls, data: Dict[str, Any], transformation: str,
                      mapping_config: Dict[str, str] = None) -> Dict[str, Any]:
        """根据转换规则转换数据"""
        ...
        for field_name, field_schema in schema.items():
            ...
            if expected_type:
                ...
                if type_validator and not type_validator['validator'](value):
                    errors.append(
                        type_validator['error_message'].format(param_name=field_name)
                    )
                    continue
...
class WorkflowExecutionService:
    ...
    def create_execution(self, workflow_id: str, user_id: int, 
        ...
        # 验证节点层级结构
        hierarchy_errors = self.node_hierarchy.validate_node_hierarchy(workflow)
        if hierarchy_errors:
    ...
    def prepare_execution_context(self, execution: AIWorkflowExecution) -> ExecutionContext:
        """准备执行上下文"""
    ...
    def execute_node(self, node: WorkflowNode, context: ExecutionContext) -> Dict[str, Any]:
        """执行单个节点（带完整错误处理）"""
        ...
        try:
            ...
            # Step 7: 记录执行结果
            self._record_node_execution(node, context, result, 'completed', None)
    ...
    def get_execution_status(self, execution_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'skipped_count': node_executions.filter(status='skipped').count()
        }
...
class WorkflowRefactoringService:
    ...
    def refactor_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        # Step 1: 分析当前工作流结构
        current_structure = self._analyze_workflow_structure(workflow)
    ...
    def _analyze_workflow_structure(self, workflow: AIWorkflow) -> Dict[str, Any]:
        """分析工作流结构"""
        nodes = list(workflow.nodes.filter(is_active=True))
    ...
    def _optimize_node_config(self, node: WorkflowNode) -> List[Dict[str, Any]]:
        """优化节点配置"""
        changes = []
    ...
    def _optimize_connection_config(self, conn: WorkflowConnection) -> List[Dict[str, Any]]:
        """优化连接配置"""
        changes = []
