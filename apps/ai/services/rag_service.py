    ...
    def _enhanced_generate_response(self, user, query, context_limit=None):
        ...
        try:
            ...
            if not retrieved_items:
                # 如果增强检索没有结果，回退到基础检索
                return self._basic_generate_response(
                    user, query, context_limit)
            ...
            response = self._generate_answer(
                rewritten_query, context, user, use_rewritten=True)
...
改写后的查询："""
    ...
    def _semantic_retrieve(self, query_vector, user, top_k=None):
        ...
        try:
            ...
            vectors = []
            ids = []
            for item_id, vector_bytes, dimension in AIKnowledgeVector.objects.filter(
                knowledge_item__in=knowledge_items
            ).values_list('knowledge_item_id', 'vector', 'dimension'):
                if dimension != int(q.size):
                    continue
                v = np.frombuffer(vector_bytes, dtype=np.float32)
                if v.size != q.size:
                    continue
                ids.append(item_id)
                vectors.append(v)
            ...
            denom = norms * q_norm
            sims = np.zeros(mat.shape[0], dtype=np.float32)
            valid = denom > 0
            if valid.any():
                sims[valid] = (mat[valid] @ q) / denom[valid]
            ...
            top_ids = [item_id for item_id, _ in candidates[:top_k]]
            sim_map = {item_id: score for item_id, score in candidates}
            items_by_id = AIKnowledgeItem.objects.in_bulk(top_ids)
            return [(items_by_id[i], sim_map.get(i, 0.0)) for i in top_ids if i in items_by_id]
        except Exception as e:
    ...
    def _merge_results(
        ...
        for item_id, data in merged.items():
            ...
            # 归一化分数
            max_semantic = max(s['semantic_score']
                               for s in semantic_results) if semantic_results else 1
            max_keyword = max(s['keyword_score']
                              for s in keyword_results) if keyword_results else 1
            ...
            # 混合权重
            hybrid_score = self.hybrid_weight * normalized_semantic + \
                (1 - self.hybrid_weight) * normalized_keyword
...
class RAGService:
    ...
    def generate_response(
        ...
        """生成RAG回复"""
        if use_enhanced:
            return self.enhanced_service.generate_response(
                user, query, context_limit)
    ...
    def _retrieve_relevant_items(self, query_vector, user):
        ...
        def slow_path(items):
            try:
                knowledge_vectors = {}
                for vector_record in AIKnowledgeVector.objects.filter(
                        knowledge_item__in=items):
                ...
                for item in items:
                    if item.id in knowledge_vectors:
                        ...
                        item_vector = self._vector_from_bytes(
                            vector_data['vector'], vector_data['dimension'])
                    else:
                        from apps.ai.services.vector_generation_service import vector_generation_service
                        item_vector = vector_generation_service.get_vector_for_knowledge_item(
                            item.id)
                ...
                return [item for item, _ in relevant_items[:self.max_relevant_items]]
            except Exception as e:
        ...
        try:
            ...
            if not vectors:
                return slow_path(knowledge_items)
            ...
            candidates.sort(key=lambda x: x[1], reverse=True)
            top_ids = [item_id for item_id, _ in candidates[:self.max_relevant_items]]
            items_by_id = AIKnowledgeItem.objects.in_bulk(top_ids)
            return [items_by_id[i] for i in top_ids if i in items_by_id]
        except Exception as e:
            logger.error(f"检索相关知识失败: {str(e)}")
            return []
    ...
    def _build_context(self, relevant_items, context_limit=None):
        ...
        for item in relevant_items:
            ...
            if context_limit and (
                    len(context) +
                    len(item_context)) > context_limit:
                break
...
"""
    ...
    def batch_generate_vectors(self, knowledge_item_ids=None):
        """批量生成向量"""
        from apps.ai.services.vector_generation_service import vector_generation_service
        return vector_generation_service.batch_generate_vectors(
            knowledge_item_ids)
