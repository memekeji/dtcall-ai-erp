...
class VectorQualityService:
    ...
    def evaluate_vector_quality(self):
        ...
        try:
            ...
            for item in knowledge_items:
                try:
                    vector_record = AIKnowledgeVector.objects.get(knowledge_item=item)
                    vector = np.frombuffer(vector_record.vector, dtype=np.float32).tolist()
    ...
    def _calculate_vector_quality(self, knowledge_item, vector):
        ...
        try:
            recreated_vector = self.ai_client.embedding(f"{knowledge_item.title}\n{knowledge_item.content}")
            metrics['consistency'] = self._calculate_cosine_similarity(vector, recreated_vector)
        except Exception as e:
        ...
        if metrics['dimension'] > 0:
            ...
            dimension_score = min(1.0, max(0.0, (metrics['dimension'] - 512) / 7168))
            ...
            # 综合得分（权重可调整）
            score = (dimension_score * 0.2) + (magnitude_score * 0.2) + (consistency_score * 0.4) + (sparsity_score * 0.2)
    ...
    def optimize_vectors(self, quality_threshold=0.7):
        ...
        try:
            ...
            # 找出低质量向量
            low_quality_items = [result for result in quality_results if result['quality_score'] < quality_threshold]
            
            # 重新生成低质量向量
            from apps.ai.services.vector_generation_service import vector_generation_service
            for item in low_quality_items:
                try:
                    vector_generation_service.generate_vector_for_knowledge_item(item['knowledge_item_id'])
                    logger.info(f"优化向量成功 - 知识条目ID: {item['knowledge_item_id']}, 标题: {item['title']}")
                except Exception as e:
                    logger.error(f"优化向量失败 - 知识条目ID: {item['knowledge_item_id']}, 错误: {str(e)}")
