...
class AIAnalysisTool:
    ...
    def analyze(self, context, task_type, **kwargs):
        ...
        try:
            ...
            # 确保ai_response是字符串格式
            analysis_content = str(ai_response) if ai_response is not None else ""
            
            # 日志记录AI响应内容，用于调试
            logger.info(f"AI分析响应内容: {analysis_content[:100]}...")
...
class CustomerAnalysisTool(AIAnalysisTool):
    ...
    def classify_customer(self, customer_data):
        ...
        result = self.analyze(context, task_type, classification_rules=classification_rules)
        return result
    ...
    def generate_customer_profile(self, customer_data, follow_records):
        ...
        result = self.analyze(context, task_type, profile_requirements=profile_requirements)
        return result
...
class ProjectAnalysisTool(AIAnalysisTool):
    ...
    def predict_risk(self, project_data, task_data, work_hours):
        ...
        result = self.analyze(context, task_type, risk_dimensions=risk_dimensions)
        return result
    
    def predict_project_risk(self, project_data, task_data, task_stats, total_hours):
        """
        项目风险预测（兼容AI视图调用）
        :param project_data: 项目基本数据
        :param task_data: 任务数据
    ...
    def analyze_project_progress(self, project_data, tasks):
        ...
        result = self.analyze(context, task_type, analysis_requirements=progress_requirements)
        return result
...
请严格按照语音转文字内容生成会议纪要，只记录实际讨论的内容。如果语音内容中没有明确提到的信息，请使用"待确认"或"待定"。"""
    ...
    def generate_meeting_minutes(self, user, meeting_id, audio_file_path=None, transcript_content=None, started_at=None, completed_at=None):
        """
        ...
        try:
            ...
            # 检查语音转文字内容是否有效
            transcription = transcript_content or meeting_data_with_audio.get('transcription', '')
            meeting_content = meeting_data_with_audio.get('meeting_content', '')
            ...
            try:
                ...
                if result['success']:
                    logger.info(f"会议纪要生成成功，使用语音转文字内容: {result.get('used_transcript', False)}")
            ...
            logger.error(f"生成会议纪要时发生错误（耗时{total_time:.2f}秒）: {str(e)}", exc_info=True)
            # 返回基于实际语音内容的错误处理
