    ...
    def __init__(self, api_key: Optional[str] = None, secret_key: Optional[str] = None):
        ...
        if secret_key:
            ...
            if db_config and db_config.get('api_key'):
                ...
                self.secret_key = db_config.get('secret_key') or getattr(settings, 'BAIDU_SECRET_KEY', None)
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('err_no') == 0:
                    ...
                    raise STTError(f"百度语音识别失败: {result.get('err_msg', '未知错误')}")
            else:
...
class LocalSTTService(STTService):
    ...
    def _convert_audio_format(self, audio_file_path: str) -> str:
        ...
        try:
            ...
            temp_wav_path = os.path.join(temp_dir, f"temp_converted_{os.path.basename(audio_file_path)}.wav")
    ...
    def transcribe_audio(self, audio_file_path: str, **kwargs) -> str:
        ...
        try:
            ...
            with sr.AudioFile(converted_path) as source:
                ...
                try:
                    text = self._recognizer.recognize_sphinx(audio_data, language='zh-CN')
...
class FreeSTTService(STTService):
    ...
    def __init__(self, service_type: str = None):
        ...
        if service_type is None:
            self.service_type = getattr(settings, 'FREE_STT_SERVICE_TYPE', 'local')
        else:
        ...
        for service_name in self._fallback_services:
            try:
                if service_name == 'local_offline':
                    ...
                    return self._transcribe_with_speech_recognition(audio_file_path, **kwargs)
                elif service_name == 'mock':
                    return self._transcribe_with_mock(audio_file_path, **kwargs)
            except Exception as e:
    ...
    def _transcribe_with_speech_recognition(self, audio_file_path: str, **kwargs) -> str:
...
def transcribe_audio_file(audio_file_path: str, service_type: str = 'auto', **kwargs) -> str:
    """
    便捷函数：使用指定服务类型转写音频文件
