...
class NotificationProcessor(BaseNodeProcessor):
    ...
    def execute(self, config: dict, context: dict) -> dict:
        ...
        try:
            if notification_type == 'email':
                ...
                self._send_system_notification(recipients, subject, message, result)
            else:
    ...
    def _send_email(self, config: dict, recipients: list, subject: str, message: str, result: dict):
        """发送邮件通知"""
    ...
    def _send_sms(self, config: dict, recipients: list, message: str, result: dict):
        """发送短信通知"""
    ...
    def _send_webhook(self, config: dict, recipients: list, message: str, result: dict):
        """发送Webhook通知"""
    ...
    def _send_system_notification(self, recipients: list, subject: str, message: str, result: dict):
        """发送系统通知"""
        for recipient in recipients:
            try:
                # 这里可以集成系统内部通知机制
    ...
    def execute(self, config: dict, context: dict) -> dict:
        ...
        try:
            if delay_type == 'fixed':
                ...
                # 计算动态延迟时间
                delay_seconds = self._evaluate_delay_expression(delay_expression, context)
                result['actual_delay'] = delay_seconds
                ...
                # 计算到目标时间的延迟
                delay_seconds = self._calculate_delay_until_time(target_time_str)
                result['actual_delay'] = delay_seconds
