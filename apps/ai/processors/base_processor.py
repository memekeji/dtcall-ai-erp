...
class BaseNodeProcessor(abc.ABC):
        ...
        if hasattr(self, 'execute_async'):
            ...
            if loop.is_running():
                ...
                future = asyncio.ensure_future(
                    self.execute_async(config, context))
        ...
        for field_name, field_schema in self.config_schema.items():
            ...
            if field_name in config:
                ...
                if expected_type and not self._validate_field_type(
                        config[field_name], expected_type):
    ...
    def generate_form_config(self) -> Dict[str, Any]:
        ...
        for field_name, field_schema in self.config_schema.items():
            field_config = self._generate_field_config(
    ...
    def _generate_field_config(self,
                               field_name: str,
                               field_schema: Dict[str,
                                                  Any],
                               parent_name: str = '') -> Dict[str,
                                                              Any]:
        ...
        if field_type == 'object' and 'properties' in field_schema:
            ...
            for prop_name, prop_schema in field_schema['properties'].items():
                field_config['properties'][prop_name] = self._generate_field_config(
                    prop_name, prop_schema, field_name)
