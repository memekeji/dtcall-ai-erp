        ...
        try:
            ...
            try:
                input_data = json.loads(
                    request.body) if request.content_type == 'application/json' else {}
            ...
            return JsonResponse(
                {
                    ...
                    'execution_time': (
                        result.completed_at -
                        result.started_at).total_seconds() if result.started_at and result.completed_at else None})

        except AIWorkflow.DoesNotExist:
            return JsonResponse(
                {'status': 'error', 'message': '工作流不存在'}, status=404)
            ...
            for node in nodes:
                ...
                if node.node_type == 'data_input' or node.node_type == 'start':
                    ...
                    if node.node_type == 'data_input':
                        ...
                        if input_type == 'text':
                            field['type'] = 'text'
                            field['default_value'] = config.get(
                                'text_config', {}).get('default_value', '')
                            ...
                            field['type'] = 'textarea'
                            field['default_value'] = config.get(
                                'json_config', {}).get('sample_json', '{}')
                        elif input_type == 'file':
                            field['type'] = 'file'
                            field['accept'] = config.get(
                                'file_config', {}).get(
                                'accept', '*/*')
                        else:
                            field['type'] = 'text'
...
class WorkflowDebugView(View):
    ...
    def get(self, request, pk):
        ...
        if not session_id:
            return JsonResponse(
                {'status': 'error', 'message': '缺少session_id'}, status=400)

        status = debugger_service.get_session_status(session_id)

        if not status:
            return JsonResponse(
                {'status': 'error', 'message': '调试会话不存在'}, status=404)
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            if not metric or threshold is None:
                return JsonResponse(
                    {'status': 'error', 'message': '参数不完整'}, status=400)
...
class WorkflowPermissionView(View):
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            if not user_id:
                return JsonResponse(
                    {'status': 'error', 'message': '缺少user_id'}, status=400)
