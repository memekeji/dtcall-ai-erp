...
class AIKnowledgeItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        need_vector = False
        if self.pk:
            old = AIKnowledgeItem.objects.filter(pk=self.pk).values(
                'title', 'content', 'status', 'file'
            ).first()
            if not old:
                need_vector = (self.status == 'published')
            else:
                old_file = str(old.get('file') or '')
                new_file = str(self.file or '')
                need_vector = (
                    self.status == 'published' and (
                        old.get('status') != 'published' or
                        old.get('title') != self.title or
                        old.get('content') != self.content or
                        old_file != new_file
                    )
                )
        else:
        ...
        # 自动设置文件类型和大小
        if self.file:
            self.file_type = self.file.name.split(
                '.')[-1].lower() if '.' in self.file.name else ''
        ...
        if need_vector:
            ...
            transaction.on_commit(
                lambda: vector_generation_service.generate_vector_for_knowledge_item(self.id)
            )
...
class AIIntentRecognition(models.Model):
    ...
    confidence_threshold = models.FloatField(
        default=0.65, verbose_name='置信度阈值')
    is_active = models.BooleanField(default=True, verbose_name='是否激活')
    training_count = models.IntegerField(
        default=0,
        verbose_name='训练次数',
        help_text='用于记录该意图被用于训练的次数')
...
class AIIntentRecognitionLog(models.Model):
    ...
    used_for_training = models.BooleanField(
        default=False, verbose_name='是否用于训练')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
...
class AIFeedback(models.Model):
    ...
    model_config_id = models.ForeignKey(
        AIModelConfig,
        on_delete=models.SET_NULL,
        null=True,
...
class AIBatchTest(models.Model):
    ...
    traffic_allocation = models.JSONField(
        verbose_name='流量分配',
...
class AIBatchTestResult(models.Model):
    ...
    positive_feedback_count = models.IntegerField(
        default=0, verbose_name='正面反馈次数')
    negative_feedback_count = models.IntegerField(
        default=0, verbose_name='负面反馈次数')
...
class WorkflowDataAccessConfig(models.Model):
    ...
    resource_identifier = models.CharField(
        max_length=200, verbose_name='资源标识符')
    operations = models.JSONField(
        verbose_name='允许的操作',
        help_text='如：["read", "write"]')
...
class AITask(models.Model):
    ...
    error_message = models.TextField(
        blank=True, null=True, verbose_name='错误信息')
