...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        # 命令模式
        parser.add_argument('mode', type=str, choices=['generate', 'update', 'optimize'],
        ...
        # 可选参数：质量阈值（仅用于优化模式）
        parser.add_argument('--threshold', type=float, default=0.7, help='向量质量阈值，低于此值的向量将被优化')
    ...
    def handle(self, *args, **options):
        ...
        try:
            if mode == 'generate':
                ...
                if ids:
                    # 解析ID列表
                    knowledge_item_ids = [int(id.strip()) for id in ids.split(',') if id.strip()]
                    result = rag_service.batch_generate_vectors(knowledge_item_ids)
                ...
                # 优化低质量向量
                result = rag_service.optimize_vectors(quality_threshold=threshold)
                if 'error' in result:
