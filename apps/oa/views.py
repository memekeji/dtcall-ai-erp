...
from .response_utils import (
    ...
    not_found_response, permission_denied_response, ajax_success_response, ajax_error_response
)
...
class MeetingView(LoginRequiredMixin, View):
    ...
    def list(self, request):
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            ...
            if params.get('host_id'):
                query &= Q(host_id=params['host_id']) | Q(host=params['host_id'])
    ...
    def retrieve(self, request, pk):
        try:
            ...
            action_items_list = meeting.action_items.split(';') if meeting.action_items else []
            audio_file_url = meeting.audio_file.url if meeting.audio_file else ''
...
class MeetingApplyView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        try:
            ...
            meeting_type = request.POST.get('meeting_type', MeetingTypeChoices.REGULAR)
            ...
            if audio_file_path:
                self._save_temp_reservation_data(reservation.id, audio_file_path, meeting_type)

            return success_response({'reservation_id': reservation.id}, '会议预订已提交，等待管理员审批')
    ...
    def _save_audio_file(self, audio_file):
        ...
        audio_dir = os.path.join(settings.MEDIA_ROOT, FileUploadConfig.AUDIO_UPLOAD_DIR)
        ...
        try:
            with open(file_path, 'wb+') as destination:
                for chunk in audio_file.chunks(chunk_size=FileUploadConfig.CHUNK_SIZE):
    ...
    def _save_temp_reservation_data(self, reservation_id, audio_file_path, meeting_type):
        """保存预订相关的临时数据"""
        temp_data_dir = os.path.join(settings.MEDIA_ROOT, 'temp_reservation_data')
...
def save_audio(request):
    ...
    try:
        ...
        transcript = _process_audio_and_generate_minutes(meeting, relative_path, request.user)
        ...
        try:
            ...
            if result:
                ...
                try:
                    ...
                    result_container['decisions'] = tool.extract_resolutions(result)
                    result_container['action_items'] = tool.extract_action_items(result)
                except Exception as e:
