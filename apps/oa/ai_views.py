...
def ai_meeting_summary(request, meeting_id):
    ...
    try:
        ...
        # 调用AI分析工具生成会议纪要
        result = default_meeting_analysis_tool.generate_meeting_summary(
            meeting_data)
        ...
        return JsonResponse(
            {'code': 500, 'msg': f'会议纪要生成失败: {str(e)}'}, status=500)
...
def ai_meeting_action_items(request, meeting_id):
    ...
    try:
        ...
        # 调用AI分析工具提取任务项
        result = default_meeting_analysis_tool.extract_action_items(
            meeting_data)
        ...
        return JsonResponse(
            {'code': 500, 'msg': f'任务项提取失败: {str(e)}'}, status=500)
