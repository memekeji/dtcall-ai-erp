...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    manager_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=User.objects.all(), source='manager')
    members = serializers.StringRelatedField(many=True, read_only=True)
    members_ids = serializers.PrimaryKeyRelatedField(
        write_only=True, many=True, queryset=User.objects.all(), source='members')
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    step_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=ProjectStep.objects.all(), source='step')
    assignee = serializers.StringRelatedField(read_only=True)
    assignee_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=User.objects.all(), source='assignee')
    participants = serializers.StringRelatedField(many=True, read_only=True)
    participants_ids = serializers.PrimaryKeyRelatedField(
        write_only=True, many=True, queryset=User.objects.all(), source='participants')
    creator = serializers.StringRelatedField(read_only=True)
...
class WorkHourSerializer(serializers.ModelSerializer):
    ...
    task_id = serializers.PrimaryKeyRelatedField(
        write_only=True, queryset=Task.objects.all(), source='task')
