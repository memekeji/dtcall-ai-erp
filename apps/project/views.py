...
class ProjectListView(LoginRequiredMixin, View):
    ...
    def get_data_list(self, request):
        ...
        if not request.user.is_superuser:
            # 构建权限查询条件
            permission_q = Q(
                creator=request.user) | Q(
                manager=request.user) | Q(
        ...
        projects = queryset.select_related(
            'category',
            'manager',
            'creator',
            'department',
            ...
            if project.customer_id and project.customer_id > 0:
                try:
                    ...
                    customer = Customer.objects.filter(
                        id=project.customer_id, delete_time=0).first()
                    if customer:
...
class ProjectDetailView(LoginRequiredMixin, View):
    ...
    def get(self, request, project_id):
        project = get_object_or_404(
            Project, id=project_id, delete_time__isnull=True)

        # 检查权限
        if not self.has_permission(request.user, project):
            return JsonResponse({'code': 1, 'msg': '没有权限查看此项目'}, json_dumps_params={
                                'ensure_ascii': False})
        ...
        tasks = Task.objects.filter(
            project=project,
            delete_time__isnull=True).select_related(
            'assignee',
            'creator')

        # 获取项目文档
        documents = ProjectDocument.objects.filter(
            project=project, delete_time__isnull=True).select_related('creator')

        # 获取工时统计
        work_hours = WorkHour.objects.filter(
            task__project=project).select_related(
            'user', 'task')
        ...
        for i in range(5, -1, -1):
            ...
            # 统计该月新增任务数
            created_count = tasks.filter(
                create_time__gte=month_start,
                create_time__lte=month_end).count()
            task_trend_data['created'].append(created_count)

            # 统计该月完成任务数
            completed_count = tasks.filter(
                status=3,
                update_time__gte=month_start,
                update_time__lte=month_end).count()
        ...
        try:
            ...
            suppliers = Supplier.objects.filter(
                id__in=order_supplier_ids, delete_time=0)[
                :10]  # 限制显示前10个供应商
    ...
    def post(self, request):
        try:
            ...
            if not all([name, code, start_date, end_date]):
                return JsonResponse({'code': 1, 'msg': '请填写所有必填字段'}, json_dumps_params={
                                    'ensure_ascii': False})

            # 验证客户关联（项目必须关联客户）
            customer_id = request.POST.get('customer_id')
            if not customer_id or customer_id == '0':
                return JsonResponse({'code': 1, 'msg': '项目必须关联客户'}, json_dumps_params={
                                    'ensure_ascii': False})

            # 检查项目编号是否重复
            if Project.objects.filter(code=code).exists():
                return JsonResponse({'code': 1, 'msg': '项目编号已存在'}, json_dumps_params={
                                    'ensure_ascii': False})
...
class ProjectEditView(LoginRequiredMixin, View):
    ...
    def get(self, request, project_id):
        ...
        # 检查权限
        if not self.has_permission(request.user, project):
            return JsonResponse({'code': 1, 'msg': '没有权限编辑此项目'}, json_dumps_params={
                                'ensure_ascii': False})
    ...
    def post(self, request, project_id):
        ...
        try:
            ...
            # 检查项目编号是否重复（排除当前项目）
            if Project.objects.filter(code=code).exclude(
                    id=project_id).exists():
                return JsonResponse({'code': 1, 'msg': '项目编号已存在'}, json_dumps_params={
                                    'ensure_ascii': False})
            ...
            return JsonResponse({'code': 1, 'msg': f'更新失败: {str(e)}'}, json_dumps_params={
                                'ensure_ascii': False})
...
class ProjectDeleteView(LoginRequiredMixin, View):
    ...
    def post(self, request, project_id):
        ...
        # 检查权限
        if not self.has_permission(request.user, project):
            return JsonResponse({'code': 1, 'msg': '没有权限删除此项目'}, json_dumps_params={
                                'ensure_ascii': False})
        ...
        try:
            ...
            return JsonResponse({'code': 1, 'msg': f'删除失败: {str(e)}'}, json_dumps_params={
                                'ensure_ascii': False})
    ...
    def post(self, request):
        try:
            ...
            if not all([title, project_id]):
                return JsonResponse({'code': 1, 'msg': '请填写标题和选择项目'}, json_dumps_params={
                                    'ensure_ascii': False})
            ...
            return JsonResponse({'code': 0, 'msg': '文档创建成功', 'data': {
                                'id': document.id}}, json_dumps_params={'ensure_ascii': False})
...
class ProjectDocumentEditView(LoginRequiredMixin, View):
    ...
    def get(self, request, doc_id):
        ...
        if not self.has_permission(request.user, document):
            return JsonResponse({'code': 1, 'msg': '没有权限编辑此文档'}, json_dumps_params={
                                'ensure_ascii': False})
...
class ProjectDocumentDeleteView(LoginRequiredMixin, View):
    ...
    def post(self, request, doc_id):
        ...
        # 检查权限
        if not self.has_permission(request.user, document):
            return JsonResponse({'code': 1, 'msg': '没有权限删除此文档'}, json_dumps_params={
                                'ensure_ascii': False})
        ...
        try:
            ...
            for category in categories:
                ...
                if hasattr(category, 'create_time'):
                    create_time_str = category.create_time.strftime(
                        '%Y-%m-%d %H:%M')
                elif hasattr(category, 'created_at'):
                    create_time_str = category.created_at.strftime(
                        '%Y-%m-%d %H:%M')
                data_list.append({
                    'id': category.id,
                    'name': category.name,
...
class ProjectCategoryEditView(LoginRequiredMixin, View):
    ...
    def post(self, request, category_id):
        try:
            ...
            if ProjectCategory.objects.filter(
                    name=name).exclude(id=category_id).exists():
                return JsonResponse({'code': 1, 'msg': '分类名称已存在'})
...
class ProjectDocumentUploadView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        from django.http import JsonResponse
        return JsonResponse({'code': 0, 'msg': 'ready'})
