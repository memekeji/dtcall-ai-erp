...
from apps.production.models import (
    ...
    ProcessRoute, BOM, BOMItem, QualityCheck, ResourceConsumption
)
...
class SchedulingOptimizerService:
    ...
    def optimize_schedule(self, plans: List[ProductionPlan] = None, 
        ...
        try:
            ...
            if strategy == 'sjf':
                scheduled = self._schedule_sjf(tasks, equipment_list, resources, start_date, end_date)
            elif strategy == 'edd':
                scheduled = self._schedule_edd(tasks, equipment_list, resources, start_date, end_date)
            elif strategy == 'priority':
                scheduled = self._schedule_priority(tasks, equipment_list, resources, start_date, end_date)
            else:
                scheduled = self._schedule_hybrid(tasks, equipment_list, resources, start_date, end_date)
            ...
            result.optimization_score = self._calculate_optimization_score(scheduled)
    ...
    def _schedule_sjf(self, tasks: List[ProductionTask],
        ...
        for task in task_queue:
            ...
            equipment = self._find_best_equipment(task, equipment_list, current_time)
            if not equipment:
                continue
            ...
            if scheduled_end <= end_date:
                scheduled.append(ScheduledTask(task, current_time, scheduled_end, equipment))
    ...
    def _schedule_priority(self, tasks: List[ProductionTask],
        ...
        """优先级调度策略"""
        scheduled = []
        task_queue = sorted(tasks, key=lambda t: (t.plan.priority if t.plan else 2, t.plan_end_date))
        current_time = start_date
    ...
    def _schedule_hybrid(self, tasks: List[ProductionTask],
        ...
        while remaining_tasks and current_time < end_date:
            ...
            for task in remaining_tasks:
                ...
                score = self._calculate_task_score(task, equipment, current_time)
            ...
            if best_task and best_equipment:
                ...
                if scheduled_end <= end_date:
                    scheduled.append(ScheduledTask(best_task, current_time, scheduled_end, best_equipment))
    ...
    def _find_best_equipment(self, task: ProductionTask,
        ...
        for equipment in equipment_list:
            ...
            if self._check_equipment_available(equipment, scheduled_time):
                load_score = self._calculate_equipment_load_score(equipment, scheduled_time)
    ...
    def _estimate_duration(self, task: ProductionTask, 
        ...
        """估算任务执行时长（小时）"""
        base_time = float(task.procedure.standard_time) if task.procedure else 1.0
    ...
    def _calculate_task_score(self, task: ProductionTask,
        ...
        load_score = self._calculate_equipment_load_score(equipment, current_time)
        score += load_score * 100
        
        return score
    
    def _calculate_optimization_score(self, scheduled: List[ScheduledTask]) -> float:
        """计算排程优化评分"""
        if not scheduled:
            return 0
        ...
        for task in scheduled:
            if task.original_task.plan and task.scheduled_end.date() > task.original_task.plan.plan_end_date:
        ...
        for eq_id, tasks_list in equipment_usage.items():
            ...
            if eq:
                ...
                equipment_load.append({
                    ...
                    'utilization_rate': min(total_hours / 160, 1.0)  # 假设月工作160小时
                })
    ...
    def simulate_schedule(self, plan: ProductionPlan,
        ...
        for task in tasks:
            duration = self._estimate_duration(task, task.equipment or Equipment.objects.filter(status=1).first())
...
class DeliveryPredictionService:
    ...
    def predict_delivery_rate(self, plan: ProductionPlan) -> Dict[str, Any]:
        ...
        progress_factor = float(in_progress_tasks.count()) / float(tasks.count()) * 30
        ...
        factors = [
            {'name': '已完成进度', 'value': f'{completed_rate:.1f}%', 'impact': 'positive'},
            {'name': '进行中任务', 'value': f'{in_progress_tasks.count()}个', 'impact': 'neutral'},
            {'name': '时间因素', 'value': f'{time_factor:.1f}%', 'impact': 'positive' if time_factor > 0 else 'negative'}
        ]
    ...
    def _calculate_time_factor(self, plan: ProductionPlan) -> float:
        ...
        total_days = (plan.plan_end_date - plan.plan_start_date).days if plan.plan_end_date else 1
        ...
        actual_progress = plan.completion_rate if hasattr(plan, 'completion_rate') else 0
