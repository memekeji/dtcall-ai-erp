...
class DataCollectorService:
    ...
    def _test_api_connection(self, data_source: DataSource) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"正在测试API数据源连接: {data_source.name} ({data_source.code})")
            ...
            response = self.session.request(
                ...
                headers=headers,
                params=params,
                data=data_source.request_body if method in [
            ...
            logger.info(
                f"API连接测试成功: {data_source.name}, 状态码: {response.status_code}")
    ...
    def _test_iot_connection(self, data_source: DataSource) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"正在测试IoT数据源连接: {data_source.name} ({data_source.code})")
            ...
            if result == 0:
                ...
                logger.warning(
                    f"IoT设备连接失败: {data_source.name} ({host}:{port})")
                return {
                    'success': False,
    ...
    def _test_mqtt_connection(self, data_source: DataSource) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time(
            ) < timeout and not userdata['connected'] and userdata['rc'] == -1:
                time.sleep(0.1)
    ...
    def _test_modbus_connection(
            self, data_source: DataSource) -> Dict[str, Any]:
        """测试Modbus连接"""
        try:
            # 这里需要安装pymodbus库
    ...
    def collect_data(self, data_source: DataSource) -> Dict[str, Any]:
        ...
        try:
            ...
            if not raw_result['success']:
                ...
                collection.duration = (
                    collection.end_time -
                    collection.start_time).total_seconds()
                collection.save()
                return raw_result
            ...
            if mappings.exists():
                processed_result = self._process_data(
                    raw_result['data'], mappings, collection)
    ...
    def _fetch_modbus_data(self, data_source: DataSource) -> Dict[str, Any]:
        ...
        try:
            ...
            for register in registers:
                ...
                function = register.get('function', 'holding')
                ...
                if not result.isError():
                    data[f"{function}_{address}"] = result.registers if hasattr(
                        result, 'registers') else result.bits
        ...
        try:
            if mapping.transform_type == 'multiply':
                ...
                if pattern:
                    ...
                    return match.group(
                        1) if match and match.groups() else value
    ...
    def _process_data(self, raw_data: Any, mappings,
