...
from apps.production.models import (
    ProductionPlan, ProductionTask, Equipment, QualityCheck,
    ResourceConsumption, MaterialIssue
)
...
class ProductionStatisticsService:
    ...
    def get_production_trend(self, start_date: datetime = None,
        ...
        while current_date <= end_date:
            ...
            for task in tasks:
                task_date = task['period'].date() if hasattr(
                    task['period'], 'date') else task['period']
                if task_date == current_date:
                    period_data['completed_count'] = task['completed_count']
                    period_data['completed_quantity'] = float(
                        task['completed_quantity'] or 0)
                    period_data['qualified_quantity'] = float(
                        task['qualified_quantity'] or 0)
                    if period_data['completed_quantity'] > 0:
                        period_data['quality_rate'] = round(
                            period_data['qualified_quantity'] /
                            period_data['completed_quantity'] * 100, 2
                        )
                    break
        ...
        for equipment in equipment_list:
            ...
            for task in tasks:
                if task.plan_end_time and task.plan_start_time:
                    duration = (
                        task.plan_end_time - task.plan_start_time).total_seconds() / 3600
                    task_hours += duration
            ...
            oee_data = self._calculate_oee_for_equipment(
    ...
    def _calculate_oee_for_equipment(self, equipment: Equipment,
        ...
        for task in tasks:
            if task.plan_end_time and task.plan_start_time:
                planned_hours += (task.plan_end_time -
                                  task.plan_start_time).total_seconds() / 3600
        ...
        for task in tasks.filter(status=2):
            if task.actual_start_time:
                ...
                running_hours += (end -
                                  task.actual_start_time).total_seconds() / 3600
    ...
    def get_labor_efficiency(self, start_date: datetime = None,
        ...
        for task in tasks:
            if task.assignee:
                ...
                stats['completed_quantity'] += float(
                    task.completed_quantity or 0)
                stats['qualified_quantity'] += float(
                    task.qualified_quantity or 0)
                if task.actual_end_time and task.actual_start_time:
                    hours = (task.actual_end_time -
                             task.actual_start_time).total_seconds() / 3600
        ...
        for user_id, stats in user_stats.items():
            ...
            if user:
                avg_output = stats['completed_quantity'] / \
                    stats['total_hours'] if stats['total_hours'] > 0 else 0
                quality_rate = (
                    stats['qualified_quantity'] /
                    stats['completed_quantity'] *
                    100) if stats['completed_quantity'] > 0 else 0
        ...
        efficiency_data.sort(
            key=lambda x: x['avg_output_per_hour'],
            reverse=True)
    ...
    def get_cost_analysis(self, start_date: datetime = None,
        ...
        total_cost = material_costs['total'] + \
            labor_costs['total'] + overhead_costs['total']
    ...
    def _get_labor_costs(self, start_date, end_date) -> Dict[str, Any]:
        ...
        for task in tasks:
            if task.actual_start_time and task.actual_end_time:
                total_hours += (task.actual_end_time -
                                task.actual_start_time).total_seconds() / 3600
