...
class ProcedureSetItem(models.Model):
    """工序集明细"""
    procedure_set = models.ForeignKey(
        ProcedureSet,
    ...
    estimated_time = models.DecimalField(
        max_digits=8, decimal_places=2, verbose_name='预估工时')
...
class ProcessRoute(models.Model):
    ...
    effective_date = models.DateField(
        null=True, blank=True, verbose_name='生效日期')
...
class ProcessRouteItem(models.Model):
    ...
    workstation = models.CharField(
        max_length=100, blank=True, verbose_name='工位')
...
class Equipment(models.Model, StatusDisplayMixin):
    ...
    manufacturer = models.CharField(
        max_length=100, blank=True, verbose_name='制造商')
    purchase_date = models.DateField(
        null=True, blank=True, verbose_name='采购日期')
    ...
    last_maintenance = models.DateField(
        null=True, blank=True, verbose_name='上次维护日期')
    next_maintenance = models.DateField(
        null=True, blank=True, verbose_name='下次维护日期')
...
class ProductionPlan(models.Model, StatusDisplayMixin):
    ...
    actual_end_date = models.DateField(
        null=True, blank=True, verbose_name='实际结束日期')
    status = models.IntegerField(
        choices=STATUS_CHOICES,
        default=1,
        verbose_name='计划状态')
    ...
    complete_threshold = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        default=100.00,
        verbose_name='自动完工阈值(%)')
    ...
    def completion_rate(self):
        """计划完成率"""
        total_completed = self.tasks.aggregate(
...
class ProductionTask(models.Model, StatusDisplayMixin, RateCalculationMixin):
    ...
    completed_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name='完成数量')
    qualified_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name='合格数量')
    defective_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name='不合格数量')
    ...
    suspended_by = models.ForeignKey(
        ...
        verbose_name='挂起人')
    suspended_time = models.DateTimeField(
        null=True, blank=True, verbose_name='挂起时间')
...
class QualityCheck(models.Model):
    ...
    check_time = models.DateTimeField(
        default=timezone.now, verbose_name='检查时间')
    created_by = models.ForeignKey(
        ...
        verbose_name='检查员')
    check_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='检查数量')
    qualified_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='合格数量')
    defective_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='不合格数量')
    ...
    def qualified_rate(self):
        """合格率"""
        return RateCalculationMixin.calculate_rate(
            self.qualified_quantity, self.check_quantity)
...
class DataCollection(models.Model):
    ...
    parameter_value = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='参数值')
    unit = models.CharField(max_length=20, verbose_name='单位')
    standard_min = models.DecimalField(
        max_digits=10,
        decimal_places=4,
        null=True,
        blank=True,
        verbose_name='标准下限')
    standard_max = models.DecimalField(
        max_digits=10,
        decimal_places=4,
        null=True,
        blank=True,
    ...
    collect_time = models.DateTimeField(
        default=timezone.now, verbose_name='采集时间')
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
...
class DataSource(models.Model):
    ...
    request_method = models.CharField(
        max_length=10, default='GET', verbose_name='请求方法')
    request_headers = models.JSONField(
        default=dict, blank=True, verbose_name='请求头')
    request_params = models.JSONField(
        default=dict, blank=True, verbose_name='请求参数')
    ...
    collection_interval = models.IntegerField(
        default=60, verbose_name='采集间隔(秒)')
    is_active = models.BooleanField(default=True, verbose_name='是否启用')
    last_collection_time = models.DateTimeField(
        null=True, blank=True, verbose_name='最后采集时间')
    last_success_time = models.DateTimeField(
        null=True, blank=True, verbose_name='最后成功时间')
...
class DataMapping(models.Model):
    ...
    transform_params = models.JSONField(
        default=dict, blank=True, verbose_name='转换参数')
    ...
    regex_pattern = models.CharField(
        max_length=500, blank=True, verbose_name='正则表达式')
    default_value = models.TextField(blank=True, verbose_name='默认值')
    target_table = models.CharField(
        max_length=100, blank=True, verbose_name='目标表')
    target_field = models.CharField(
        max_length=100, blank=True, verbose_name='目标字段')
    sort = models.IntegerField(default=0, verbose_name='排序')
    is_active = models.BooleanField(default=True, verbose_name='是否启用')
    create_time = models.DateTimeField(
        default=timezone.now, verbose_name='创建时间')
...
class DataCollectionRecord(models.Model):
    ...
    processed_data = models.JSONField(
        default=dict, blank=True, verbose_name='处理后数据')
    ...
    error_details = models.JSONField(
        default=list, blank=True, verbose_name='错误详情')
...
class ProductionDataPoint(models.Model):
    ...
    metric_unit = models.CharField(
        max_length=50, blank=True, verbose_name='单位')
...
class DataCollectionTask(models.Model):
    ...
    cron_expression = models.CharField(
        max_length=100, blank=True, verbose_name='Cron表达式')
    ...
    last_run_time = models.DateTimeField(
        null=True, blank=True, verbose_name='最后执行时间')
...
class MaterialRequestItem(models.Model):
    ...
    specification = models.CharField(
        max_length=100, blank=True, verbose_name='规格型号')
    unit = models.CharField(max_length=20, verbose_name='单位')
    request_quantity = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='申请数量')
...
class MaterialIssueItem(models.Model):
    ...
    issue_quantity = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='出库数量')
    unit_cost = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
...
class MaterialReturnItem(models.Model):
    ...
    return_quantity = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='退料数量')
    unit_cost = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        default=0,
...
class WorkCompletionReport(models.Model):
    ...
    reported_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='申报数量')
    qualified_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='合格数量')
    defective_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='不合格数量')
    ...
    work_hours = models.DecimalField(
        ...
        default=0,
        verbose_name='实际工时')
    resource_consumption = models.JSONField(
        default=dict, blank=True, verbose_name='资源消耗')
...
class WorkCompletionRedFlush(models.Model):
    ...
    completion_report = models.ForeignKey(
        WorkCompletionReport,
        on_delete=models.CASCADE,
        related_name='red_flushes',
        verbose_name='完工申报单')
    red_flush_date = models.DateField(
        default=timezone.now, verbose_name='红冲日期')
    red_flush_reason = models.TextField(verbose_name='红冲原因')
    red_flush_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='红冲数量')
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
...
class ProductReceipt(models.Model):
    ...
    receipt_quantity = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='入库数量')
...
class OrderMaterialConfirmation(models.Model):
    ...
    confirmed_quantity = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='确认数量')
    ...
    confirm_time = models.DateTimeField(
        default=timezone.now, verbose_name='确认时间')
    remarks = models.TextField(blank=True, verbose_name='备注')
...
class ResourceConsumption(models.Model):
    ...
    consumed_quantity = models.DecimalField(
        max_digits=10, decimal_places=4, verbose_name='消耗数量')
    unit = models.CharField(max_length=20, verbose_name='单位')
    ...
    consumption_time = models.DateTimeField(
        default=timezone.now, verbose_name='消耗时间')
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
