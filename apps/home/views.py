...
def _build_menu_tree(available_menus, user_permissions=None, is_superuser=False):
    ...
    def _check_permission(codename):
        ...
        full_perm = f'user.{codename}' if not codename.startswith('user.') else codename
...
def dashboard(request):
    ...
    if cached_dashboard_data:
        ...
        message_count = MessageUserRelation.objects.filter(user=request.user, is_read=False).count()
        ...
        try:
            if is_superuser:
                ...
                total_users = User.objects.filter(is_active=True, groups__admin=request.user).distinct().count()
                ...
                total_contracts = Contract.objects.filter(contract_filter).count()
                ...
                source_counts = Customer.objects.filter(customer_filter).values('customer_source').annotate(count=Count('id'))
                source_ids = [item['customer_source'] for item in source_counts if item['customer_source'] is not None]
                sources = {s.id: s.title for s in CustomerSource.objects.filter(id__in=source_ids)}
                pie_data = [{'name': sources.get(item['customer_source'], '未知来源'), 'value': item['count']} for item in source_counts if item['customer_source'] is not None]
    ...
    try:
        if is_superuser:
            ...
            ongoing_projects = Project.objects.filter(delete_time__isnull=True, status=2, manager=request.user).count()
    except Exception as e:
    ...
    try:
        if is_superuser:
            ...
            total_customers_count = Customer.objects.filter(customer_filter).count()
            customers_with_orders = CustomerOrder.objects.filter(
                delete_time=0,
                status__in=['confirmed', 'processing', 'shipped', 'delivered', 'completed'],
                customer__in=Customer.objects.filter(customer_filter)
    ...
    try:
        if is_superuser:
            ...
            projects = Project.objects.filter(delete_time__isnull=True, status=2, manager=request.user).order_by('-progress')[:5]
    ...
    try:
        if is_superuser:
            total_projects = Project.objects.filter(delete_time__isnull=True).count()
        else:
            total_projects = Project.objects.filter(delete_time__isnull=True, manager=request.user).count()
    except Exception as e:
