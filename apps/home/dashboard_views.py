...
from django.db.models import Count, Sum, Q, Value as models_Value, CharField, Case, When, IntegerField
...
def dashboard(request):
    ...
    try:
        # 获取真实业务数据
        from apps.customer.models import Customer, CustomerSource, CustomerContract
        ...
        # 客户来源分布
        source_data = Customer.objects.filter(delete_time=0).values('customer_source__title').annotate(
            count=Count('id')
        ).order_by('-count')[:6]
        ...
        try:
            # 查询真实的项目数据，使用正确的状态值
            projects = Project.objects.filter(delete_time__isnull=True, status=2)[:5]  # status=2表示进行中
    ...
    try:
        ...
        # 修正客户数统计：用户客户 + 公海客户
        user_customers_count = Customer.objects.filter(delete_time=0, belong_uid__gt=0).count()
        public_customers_count = Customer.objects.filter(delete_time=0, belong_uid=0).count()
...
def finance_dashboard(request):
    ...
    try:
        ...
        revenue_growth = round((revenue - revenue_last) / max(revenue_last, 1) * 100, 1) if revenue_last > 0 else 0
        
        # 毛利 = 收入 - 成本（约20%成本率）
        gross_profit_rate = 0.8  # 假设毛利率80%
        gross_profit = revenue * gross_profit_rate
        gross_profit_last = revenue_last * gross_profit_rate
        gross_profit_growth = round((gross_profit - gross_profit_last) / max(gross_profit_last, 1) * 100, 1) if gross_profit_last > 0 else 0
        ...
        expense_growth = round((expense - expense_last) / max(expense_last, 1) * 100, 1) if expense_last > 0 else 0
        ...
        for i in range(12):
            ...
            profit_trend_data.append(month_income * gross_profit_rate - float(month_exp))
        ...
        overdue_amount = Invoice.objects.filter(
            create_time__gte=int((current_month - timedelta(days=30)).timestamp()),
            create_time__lt=current_month_ts,
            enter_status__in=[0, 1]
        ).aggregate(total=Sum('amount') - Sum('enter_amount'))['total'] or 0
        ...
        try:
            ...
            overdue_count = Invoice.objects.filter(
                enter_status__in=[0, 1],
                create_time__lt=int((current_month - timedelta(days=30)).timestamp())
            ).count()
            if overdue_count > 0:
        ...
        expense_growth = 0
        net_profit_growth = 0
        income_expense_trend = {'labels': '[]', 'income': '[]', 'expense': '[]'}
...
def business_dashboard(request):
    ...
    try:
        ...
        # 增长率
        new_customers_growth = round((new_customers - new_customers_last) / max(new_customers_last, 1) * 100, 1) if new_customers_last > 0 else 0
        ...
        signed_customers_growth = round((signed_customers - signed_customers_last) / max(signed_customers_last, 1) * 100, 1) if signed_customers_last > 0 else 0
        
        # 客户满意度（从系统配置获取或计算）
        try:
            from apps.user.models import SystemConfiguration
            sat_cfg = SystemConfiguration.objects.filter(key='customer_satisfaction', is_active=True).first()
        ...
        try:
            from apps.user.models import SystemConfiguration
            sat_cfg_last = SystemConfiguration.objects.filter(key='customer_satisfaction_last', is_active=True).first()
            satisfaction_last = float(sat_cfg_last.value) if sat_cfg_last else customer_satisfaction
            satisfaction_growth = round(customer_satisfaction - satisfaction_last, 1)
        except:
        ...
        try:
            from apps.user.models import SystemConfiguration
            share_cfg = SystemConfiguration.objects.filter(key='market_share', is_active=True).first()
        ...
        try:
            from apps.user.models import SystemConfiguration
            share_cfg_last = SystemConfiguration.objects.filter(key='market_share_last', is_active=True).first()
            share_last = float(share_cfg_last.value) if share_cfg_last else market_share
        ...
        interested_customers = Customer.objects.filter(delete_time=0, services_id__in=[2, 3]).count()
        opportunity_customers = Customer.objects.filter(delete_time=0, services_id__in=[4, 6]).count()
        ...
        try:
            ...
            cac = round(marketing_expense / max(new_customers, 1), 2) if new_customers > 0 else 0
        except:
            cac = 0
        ...
        try:
            ...
            ltv = round(avg_order_amount * avg_orders_per_customer * 24, 2)  # 假设24个月生命周期
        except:
            ltv = 0
        ...
        funnel_data = {'potential': 0, 'interested': 0, 'opportunity': 0, 'signed': 0}
        sales_ranking = []
        industry_data = {'labels': '[]', 'data': '[]'}
        cac = 0
        ltv = 0
...
def production_dashboard(request):
    ...
    try:
        ...
        monthly_plans_growth = round((monthly_plans - monthly_plans_last) / max(monthly_plans_last, 1) * 100, 1) if monthly_plans_last > 0 else 0
        ...
        completed_tasks_growth = round((completed_tasks - completed_tasks_last) / max(completed_tasks_last, 1) * 100, 1) if completed_tasks_last > 0 else 0
        ...
        equipment_utilization = round((active_equipment / max(total_equipment, 1)) * 100, 1)
        
        # 上月设备利用率
        equipment_utilization_last = equipment_utilization
        equipment_utilization_growth = round(equipment_utilization - equipment_utilization_last, 1)
        
        # 生产效率 = 完成任务数 / 总任务数
        total_tasks = ProductionTask.objects.count()
        completed_tasks_total = ProductionTask.objects.filter(status=3).count()
        production_efficiency = round((completed_tasks_total / max(total_tasks, 1)) * 100, 1) if total_tasks > 0 else 0
        
        # 上月生产效率
        total_tasks_last = ProductionTask.objects.count()
        completed_tasks_total_last = ProductionTask.objects.filter(status=3).count()
        production_efficiency_last = round((completed_tasks_total_last / max(total_tasks_last, 1)) * 100, 1) if total_tasks_last > 0 else 0
        production_efficiency_growth = round(production_efficiency - production_efficiency_last, 1)
        ...
        try:
            ...
            for line in lines:
                ...
                production_lines.append({
                    ...
                    'status': 'running' if line.status == 1 else ('maintenance' if line.status == 2 else ('error' if line.status == 3 else 'stopped')),
                    'planned': total_plan,
                    'produced': total_produced,
                    'efficiency': round((total_produced / max(total_plan, 1)) * 100, 1) if total_plan > 0 else 0,
                    'quality_rate': 98.5,
