...
def mask_sensitive_value(value: str, mask_char: str = '*', visible_count: int = 2) -> str:
    """
    遮蔽字符串中的敏感信息
    ...
    return value[:visible_count] + mask_char * (length - visible_count * 2) + value[-visible_count:]
...
def mask_sensitive_field(field_name: str, field_value: Any) -> Any:
    ...
    if field_lower in SENSITIVE_FIELDS:
        if isinstance(field_value, str):
            ...
            return {k: mask_sensitive_field(f"{field_name}_{k}", v) for k, v in field_value.items()}
        elif isinstance(field_value, list):
            return [mask_sensitive_field(field_name, item) for item in field_value]
        return field_value
    
    if isinstance(field_value, str):
        for pattern in SENSITIVE_PATTERNS:
            field_value = re.sub(pattern, lambda m: f"{m.group(1)}{mask_sensitive_value(m.group(2))}", field_value)
...
def sanitize_log_content(content: Any) -> Any:
    ...
    if isinstance(content, str):
        result = content
        for pattern in SENSITIVE_PATTERNS:
            result = re.sub(pattern, lambda m: mask_sensitive_value(m.group(0)), result)
        return result
...
def sanitize_log_record(record: Dict[str, Any], sensitive_fields: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    清理单条日志记录中的敏感信息
...
def sanitize_log_list(records: List[Dict[str, Any]], sensitive_fields: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    """
    批量清理日志记录中的敏感信息
    ...
    Returns:
        清理后的日志记录列表
    """
    return [sanitize_log_record(record, sensitive_fields) for record in records]
...
def sanitize_dict_for_log(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if any(sensitive in key_lower for sensitive in SENSITIVE_FIELDS):
            ...
            result[key] = value
            for pattern in SENSITIVE_PATTERNS:
                result[key] = re.sub(pattern, lambda m: mask_sensitive_value(m.group(0)), result[key])
        elif isinstance(value, dict):
            result[key] = sanitize_dict_for_log(value)
