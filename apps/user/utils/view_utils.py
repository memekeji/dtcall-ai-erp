...
class GenericCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'code': 200, 'msg': '创建成功', 'data': {'id': self.object.id}})
        
        return super().form_valid(form)
    
    def form_invalid(self, form):
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'code': 400, 'msg': '创建失败', 'errors': form.errors})
...
class GenericUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'code': 200, 'msg': '更新成功', 'data': {'id': self.object.id}})
        
        return super().form_valid(form)
    
    def form_invalid(self, form):
        if self.request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'code': 400, 'msg': '更新失败', 'errors': form.errors})
