...
class EmployeeForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'entry_time': forms.NumberInput(
                attrs={
                    'class': 'layui-input',
                    'placeholder': '请输入入职日期(YYYYMMDD格式)'}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['did'].widget.choices = [
            (0, '请选择部门')] + [
            (dept.id, dept.name) for dept in Department.objects.filter(
                status=1).order_by('sort')]
        self.fields['position_id'].widget.choices = [
            (0, '请选择岗位')] + [
            (pos.id, pos.title) for pos in Position.objects.filter(
                status=1).order_by('sort')]
        self.fields['pid'].widget.choices = [
            (0, '无上级')] + [
            (admin.id, admin.name) for admin in Admin.objects.filter(
        ...
        if self.instance.pk:
            ...
            if hasattr(self.instance,
                       'position_id') and self.instance.position_id:
    ...
    def save(self, commit=True):
            ...
            if isinstance(self.cleaned_data['entry_time'], str):
                ...
                try:
                    entry_date = datetime.strptime(
                        self.cleaned_data['entry_time'], '%Y-%m-%d')
                    instance.entry_time = int(entry_date.strftime('%Y%m%d'))
                except ValueError:
                    # 如果日期格式不正确，尝试直接转换为整数
                    try:
                        instance.entry_time = int(
                            self.cleaned_data['entry_time'])
                    except (ValueError, TypeError):
                        # 如果转换失败，设置为0
                        instance.entry_time = 0
            elif hasattr(self.cleaned_data['entry_time'], 'strftime'):
                instance.entry_time = int(
                    self.cleaned_data['entry_time'].strftime('%Y%m%d'))
            else:
                # 直接使用整数值
                try:
        ...
        if commit:
            ...
            if hasattr(instance, 'secondary_departments'):
                instance.secondary_departments.set(
