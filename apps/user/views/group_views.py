...
class GroupListAPIView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        for group in groups:
            ...
            # 获取分配的部门
            departments = DepartmentGroup.objects.filter(
                group=group).select_related('department')
...
class GroupCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # 自动创建GroupExtension
        GroupExtension.objects.get_or_create(
            group=self.object, defaults={'status': True})
        return response


class GroupUpdateViewCBV(
...
class GetGroupsAPIView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # 返回JSON响应
        return JsonResponse({'status': 'success', 'data': groups_list},
...
class RoleListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 处理isInIframe参数
        context['isInIframe'] = self.request.GET.get(
            'isInIframe', 'false').lower() == 'true'
        ...
        for role in roles:
            ...
            # 获取分配的部门
            departments = DepartmentGroup.objects.filter(
                group=role).select_related('department')
