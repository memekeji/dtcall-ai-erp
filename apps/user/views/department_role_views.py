...
class DepartmentRoleManagementView(
    ...
    def get(self, request, department_id):
        ...
        try:
            ...
            # 获取部门已分配的角色
            department_roles = DepartmentGroup.objects.filter(
                department=department)
            department_role_ids = [dg.group.id for dg in department_roles]

            # 获取部门员工数
            from apps.user.models import Admin
            staff_count = Admin.objects.filter(
                did=department_id, status=1).count()
    ...
    def post(self, request, department_id):
        ...
        try:
            ...
            # 获取选中的角色ID列表
            selected_role_ids = request.POST.getlist(
                'department_role_ids[]', [])
            selected_role_ids = [
                int(rid) for rid in selected_role_ids if rid.isdigit()]
            ...
            for role_id in selected_role_ids:
                try:
                    ...
                    DepartmentGroup.objects.create(
                        department=department, group=role)
...
class DepartmentRoleListAPIView(
        LoginRequiredMixin, PermissionRequiredMixin, View):
    """获取部门角色列表API"""
    ...
    def get(self, request, department_id):
        ...
        try:
            ...
            # 获取部门已分配的角色
            department_roles = DepartmentGroup.objects.filter(
                department=department).select_related('group')
            ...
            return JsonResponse(
                {'code': 200, 'msg': '获取成功', 'data': roles_data})
        except Department.DoesNotExist:
            return JsonResponse({'code': 404, 'msg': '部门不存在'})
