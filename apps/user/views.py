    ...
    def post(self, request, *args, **kwargs):
        if request.headers.get('x-requested-with') == 'XMLHttpRequest':
            try:
                ...
                if Department.objects.filter(name=title).exclude(
                        id=department.id).exists():
                    return JsonResponse({
                        'code': 1,
                        'msg': '部门名称已存在',
                        'errors': {'title': ['该部门名称已被使用']}
...
class AdminListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if sort_field and sort_field.lstrip(
                '-') in ['username', 'name', 'create_time']:
...
class AdminLogListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        sort_field = self.request.GET.get('sort')
        if sort_field and sort_field.lstrip(
                '-') in ['create_time', 'admin__username', 'action']:
            queryset = queryset.order_by(sort_field)
        else:
