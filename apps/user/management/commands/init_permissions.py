...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['dry_run']:
            ...
            self.stdout.write(self.style.WARNING('=== 警告：强制模式将删除现有权限并重新创建 ==='))
            confirm = input('\n确认继续？(yes/no): ')
    ...
    def _create_child_permissions(self, module_name, child_key, child_config, content_type):
        """创建子菜单权限"""
        created = 0
        existed = 0
    ...
    def _create_sub_permissions(self, module_name, parent_name, sub_key, sub_config, content_type):
        """创建三级子菜单权限"""
        created = 0
        existed = 0
    ...
    def _get_or_create_permission(self, codename, name, content_type):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'  权限创建跳过 ({codename}): {str(e)}'))
            return False, False
