    ...
    def run_backup(self, policy):
        ...
        try:
            ...
            full_backup_path = os.path.join(
                settings.MEDIA_ROOT, backup_file_path)
            ...
            with connection.cursor() as cursor:
                ...
                with open(full_backup_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(
                        f"-- 数据库: {settings.DATABASES['default']['NAME']}\n")
                    ...
                    for seq in sequences:
                        ...
                        try:
                            # 直接从序列获取当前值
                            cursor.execute(
                                f"SELECT last_value FROM public.{seq_name}")
                            last_value = cursor.fetchone()[0]
                            # 生成简单的序列创建语句
                            f.write(
                                f"CREATE SEQUENCE public.{seq_name} START WITH {last_value};")
                            f.write("\n")
                    ...
                    for table in tables:
                        ...
                        cursor.execute(
                            f"SELECT column_name, data_type, is_nullable, column_default, character_maximum_length, numeric_precision, numeric_scale FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '{table_name}' ORDER BY ordinal_position")
                        ...
                        if columns:
                            ...
                            # 获取主键约束
                            cursor.execute(
                                f"SELECT kcu.column_name FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu ON tc.constraint_name = kcu.constraint_name WHERE tc.table_schema = 'public' AND tc.table_name = '{table_name}' AND tc.constraint_type = 'PRIMARY KEY' ORDER BY kcu.ordinal_position")
                    ...
                    for table in tables:
                        ...
                        try:
                            ...
                            # 获取表的列名
                            cursor.execute(
                                f"SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '{table_name}' ORDER BY ordinal_position")
                            ...
                            for row in rows:
                                ...
                                f.write(
                                    f"INSERT INTO public.{table_name} ({column_names}) VALUES ({values_str});\n")
                            f.write("\n")
                        except Exception as e:
                            # 如果备份表数据失败，记录警告并跳过
            ...
            self.stdout.write(self.style.SUCCESS(
                f'备份成功，文件大小: {file_size}字节，备份ID: {backup.id}'))
        ...
        if backups.count() > policy.keep_count:
            ...
            for backup in old_backups:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'删除旧备份: {backup.name}'))
                except Exception as e:
