...
class BackupPolicy(models.Model):
    ...
    hour = models.IntegerField(
        default=2,
        verbose_name='小时',
        help_text='0-23，每天执行的小时')
    minute = models.IntegerField(
        default=0,
        verbose_name='分钟',
        help_text='0-59，每天执行的分钟')
    # 每周执行的星期几（0-6，0表示周日）
    week_day = models.IntegerField(
        default=0,
        verbose_name='星期',
        help_text='0-6，每周执行的星期几，0表示周日')
    # 每月执行的日期（1-31）
    month_day = models.IntegerField(
        default=1,
        verbose_name='日期',
        help_text='1-31，每月执行的日期')
    # 保留最新备份份数
...
class Asset(models.Model):
    ...
    purchase_price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name='购买价格')
...
class AssetRepair(models.Model):
    ...
    reporter = models.ForeignKey(
        ...
        verbose_name='报修人')
    repair_person = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    start_time = models.DateTimeField(
        null=True, blank=True, verbose_name='开始维修时间')
    ...
    def __str__(self):
        asset_name = self.asset.name if self.asset else '未知资产'
        return f"{asset_name} - {self.get_status_display()}"
...
class Vehicle(models.Model):
    ...
    status = models.CharField(
        ...
        default='normal',
        verbose_name='状态')
    insurance_expire = models.DateField(
        null=True, blank=True, verbose_name='保险到期日')
    annual_inspection = models.DateField(
        null=True, blank=True, verbose_name='年检日期')
...
class VehicleMaintenance(models.Model):
    ...
    next_maintenance = models.DateField(
        null=True, blank=True, verbose_name='下次保养日期')
...
class VehicleOil(models.Model):
    ...
    vehicle = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name='车辆')
    oil_amount = models.DecimalField(
        max_digits=8,
        decimal_places=2,
        verbose_name='加油量(升)')
...
class Notice(models.Model):
    ...
    target_departments = models.ManyToManyField(
        'department.Department',
        through='NoticeTargetDepartment',
        blank=True,
        verbose_name='目标部门')
    target_users = models.ManyToManyField(
        User,
        through='NoticeTargetUser',
        blank=True,
        verbose_name='目标用户')
...
class Document(models.Model):
    ...
    effective_time = models.DateTimeField(
        null=True, blank=True, verbose_name='生效时间')
    expire_time = models.DateTimeField(
        null=True, blank=True, verbose_name='失效时间')

    # 接收范围
    target_departments = models.ManyToManyField(
        'department.Department',
        through='DocumentTargetDepartment',
        blank=True,
        related_name='received_documents',
        verbose_name='接收部门')
    target_users = models.ManyToManyField(
        User,
        through='DocumentTargetUser',
        blank=True,
        related_name='received_documents',
        verbose_name='接收人员')
...
class StorageConfiguration(models.Model):
    ...
    nas_host = models.CharField(
        max_length=200,
        blank=True,
        verbose_name='NAS主机地址')
    nas_port = models.IntegerField(default=0, verbose_name='NAS端口')
    nas_share_path = models.CharField(
        max_length=200, blank=True, verbose_name='共享路径')
    webdav_url = models.CharField(
        max_length=500,
        blank=True,
        verbose_name='WebDAV地址')
    webdav_username = models.CharField(
        max_length=100, blank=True, verbose_name='WebDAV用户名')
    webdav_password = models.CharField(
        max_length=100, blank=True, verbose_name='WebDAV密码')
    ...
    def save(self, *args, **kwargs):
        if self.is_default:
            StorageConfiguration.objects.filter(
                is_default=True).exclude(
                pk=self.pk).update(
                is_default=False)
...
class ServiceConfiguration(models.Model):
    ...
    template_id = models.CharField(
        max_length=100, blank=True, verbose_name='模板ID')
    sign_name = models.CharField(
        max_length=100,
        blank=True,
        verbose_name='签名名称')

    request_rate_limit = models.IntegerField(
        default=0, verbose_name='请求频率限制(次/秒)')
    max_audio_size = models.BigIntegerField(
        default=0, verbose_name='最大音频大小(字节)')
    supported_formats = models.CharField(
        max_length=200, blank=True, verbose_name='支持格式')
...
def create_meeting_record(sender, instance, created, **kwargs):
    if instance.status == 'approved' and not hasattr(
            instance, 'meeting_record'):
        audio_file_path = ''
        ...
        meeting_record = MeetingRecord.objects.create(
            ...
            department=instance.organizer.did if hasattr(
                instance.organizer, 'did') and instance.organizer.did else None,
