...
class StorageConfigFormView(BaseAdminView, View):
    ...
    def post(self, request, pk=None):
        try:
            ...
            config.max_file_size = int(
                max_file_size) * 1024 * 1024 if max_file_size else 0
            config.allowed_extensions = request.POST.get(
                'allowed_extensions', '')
    ...
    def _test_config_from_form(self, request):
        ...
        try:
            ...
            if storage_type in ['aliyun', 'tencent',
                                'huawei', 'baidu', 'qiniu', 'aws']:
                ...
                return storage_service.test_nas_storage(nas_host, int(
                    request.POST.get('nas_port', 0) or 0), nas_share_path)
    ...
    def post(self, request):
        try:
            ...
            if storage_type == 'local':
                return JsonResponse(
                    {'status': 'success', 'message': '本地存储无需测试连接'})
            ...
            if storage_type in ['aliyun', 'tencent',
                ...
                if not access_key or not secret_key or not bucket_name:
                    return JsonResponse(
                        {'status': 'error', 'message': '请填写完整的云存储配置信息'})

                result = storage_service.test_cloud_storage(
                    storage_type, access_key, secret_key, bucket_name, region, endpoint
                )
                return JsonResponse(
                    {'status': 'success' if result['success'] else 'error', 'message': result['message']})
                ...
                if not nas_host or not nas_share_path:
                    return JsonResponse(
                        {'status': 'error', 'message': '请填写完整的NAS配置信息'})

                result = storage_service.test_nas_storage(
                    nas_host, nas_port, nas_share_path)
                return JsonResponse(
                    {'status': 'success' if result['success'] else 'error', 'message': result['message']})
                ...
                if not webdav_url:
                    return JsonResponse(
                        {'status': 'error', 'message': '请填写WebDAV地址'})

                result = storage_service.test_webdav_storage(
                    webdav_url, webdav_username, webdav_password)
                return JsonResponse(
                    {'status': 'success' if result['success'] else 'error', 'message': result['message']})

            return JsonResponse(
                {'status': 'error', 'message': f'不支持的存储类型: {storage_type}'})
    ...
    def post(self, request, pk):
        try:
            ...
            if new_status in ['active', 'inactive']:
                ...
                config.save()
                messages.success(
                    request, f'状态已更新为"{config.get_status_display()}"')
            else:
