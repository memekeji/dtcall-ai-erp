    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'code': 1, 'msg': '请先登录'}, json_dumps_params={
                                    'ensure_ascii': False})
...
class BaseAdminView(CustomLoginRequiredMixin,
                    PermissionRequiredMixin, ModuleActiveCheckMixin):
    """系统管理视图基类"""
    ...
    def get_context_data(self, **kwargs):
        ...
        for menu in all_top_menus:
            if menu.is_available():
                # 过滤子菜单
                menu.submenus_list = [
                    submenu for submenu in menu.submenus.all() if submenu.is_available()]
