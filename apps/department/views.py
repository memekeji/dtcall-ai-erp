...
class DepartmentListView(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        def build_department_tree(parent_id=0, level=0):
            ...
            current_departments = queryset.filter(
                pid=parent_id).order_by('sort', 'id')
    ...
    def get_context_data(self, ** kwargs):
        ...
        # 将部门树形数据序列化为JSON字符串
        context['department_tree_json'] = json.dumps(
            department_tree, cls=DjangoJSONEncoder)
    ...
    try:
        ...
        if department.pid:
            parent_department = Department.objects.filter(
                id=department.pid).first()

        # 获取子部门列表
        children_departments = Department.objects.filter(
            pid=pk).order_by(
            'sort', 'id')
...
def department_delete(request, department_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # 检查是否有子部门
            has_children = Department.objects.filter(
                pid=department_id).exists()
            if has_children:
...
def department_list_api(request):
    ...
    for dept in departments:
        ...
        if dept.manager:
            ...
            if hasattr(
                    dept.manager,
                    'name') and dept.manager.name and dept.manager.name.strip():
        ...
        # 获取部门已分配的角色
        department_group_roles = DepartmentGroup.objects.filter(
            department=dept)
        # 获取角色名称列表
...
def department_tree_api(request):
    ...
    def build_tree(parent_id=0):
        ...
        current_departments = departments.filter(
            pid=parent_id).order_by('sort', 'id')
        for dept in current_departments:
            # 递归获取子部门
...
class DepartmentManagersView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        manager_list = [
            # 优先显示姓名，如果没有则显示用户名
            {'id': manager.id, 'name': manager.name or manager.username}
            for manager in managers
        ]

        return JsonResponse(
            {'code': 0, 'msg': 'success', 'data': manager_list})
...
class ManagerSelectView(PermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 获取所有用户数据
        from apps.user.models import Admin
        users = Admin.objects.filter(status=1).values(
            'id', 'username', 'name', 'mobile', 'did')
