...
class ArchivePreviewHandler:
        ...
        try:
            ...
            if file_ext == '.zip':
                ...
                return cls._create_error_response(
                    file_name, f'不支持的压缩包格式: {file_ext}')
                        ...
                        files.append(
                            {
                                ...
                                'size': cls._format_size(
                                    member.size) if member.isfile() else '-',
                                'size_bytes': member.size if member.isfile() else 0,
                                'is_dir': member.isdir(),
                                'date': datetime.fromtimestamp(
                                    member.mtime).strftime('%Y-%m-%d %H:%M') if member.isfile() else None,
    ...
    def _create_error_response(
            cls, file_name: str, message: str) -> Dict[str, Any]:
        """创建错误响应"""
        return {
            'type': 'archive',
            'name': file_name,
