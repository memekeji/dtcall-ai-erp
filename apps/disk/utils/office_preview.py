    ...
    def _extract_image(self, zf, name: str):
        ...
        try:
            ...
            try:
                ...
                img.save(
                    output,
                    format=img_format if img_format != 'JPEG' else 'JPEG',
                    quality=self.image_quality,
                    optimize=True)

                b64_data = base64.b64encode(output.getvalue()).decode('utf-8')
                mime_type = f'image/{ext.lower()}' if ext in [
                    'png', 'jpeg', 'jpg', 'gif', 'bmp', 'webp'] else 'image/png'
...
class PPTStyleExtractor:
    ...
    def extract_font_style(font) -> dict:
        ...
        if not font:
            return {}
        return {
            'name': font.name if hasattr(
                font, 'name') and font.name else None, 'size': font.size.pt if font.size and hasattr(
                font.size, 'pt') else None, 'color': PPTStyleExtractor.rgb_to_hex(
                font.color) if hasattr(
                    font, 'color') and font.color else None, 'bold': font.bold if hasattr(
                        font, 'bold') else False, 'italic': font.italic if hasattr(
                            font, 'italic') else False, 'underline': font.underline if hasattr(
                                font, 'underline') and font.underline else False}
    ...
    def extract_paragraph_style(para) -> dict:
        ...
        return {
            'alignment': str(
                para.alignment) if para.alignment else None,
            'space_before': para.space_before.pt if para.space_before and hasattr(
                para.space_before,
                'pt') else 0,
            'space_after': para.space_after.pt if para.space_after and hasattr(
                para.space_after,
                'pt') else 0,
            'level': para.level if hasattr(
                para,
                'level') else 0}
    ...
    def extract_shape_style(shape) -> dict:
        ...
        try:
            if hasattr(shape, 'fill') and shape.fill:
                ...
                if fill.type != 1:
                    fill_style['color'] = PPTStyleExtractor.rgb_to_hex(
                        fill.fore_color)
            ...
            if hasattr(shape, 'line') and shape.line:
                line = shape.line
                style['line'] = {
                    'color': PPTStyleExtractor.rgb_to_hex(
                        line.color), 'width': line.width.pt if line.width and hasattr(
                        line.width, 'pt') else 0}
            ...
            if hasattr(slide, 'background') and slide.background:
                ...
                if hasattr(bg, 'fill') and bg.fill:
                    ...
                    if fill.type == 1:
                        ...
                        background['color'] = PPTStyleExtractor.rgb_to_hex(
                            fill.fore_color)
        except Exception as e:
...
class OfficePreviewHandler:
    ...
    @classmethod
    def preview_office_file(
            cls,
            file_path: str,
            conversion_format: str = None) -> dict:
        ...
        try:
            ...
            if not office_type:
                return cls._create_error_response(
                    file_name, f'不支持的Office文件类型: {file_ext}')

            logger.info(f"文件类型: {office_type}, 扩展名: {file_ext}")

            # 如果指定了转换格式，执行转换
            if conversion_format:
                return cls._convert_office_file(
                    file_path, file_name, office_type, file_ext, conversion_format)

            # 3. 根据文件类型选择处理方法
            preview_result = cls._process_office_file(
                file_path, file_name, office_type, file_ext)
    ...
    def _convert_office_file(
            ...
            office_type: str,
            file_ext: str,
            conversion_format: str) -> dict:
        """转换Office文档到指定格式"""
        try:
            # 检查转换格式是否支持
            supported_formats = cls.CONVERSION_OPTIONS
            if conversion_format not in supported_formats or not supported_formats[
                    conversion_format]:
                return cls._create_error_response(
                    file_name, f'不支持将{office_type}文档转换为{conversion_format}格式')

            # 执行转换
            if conversion_format == 'html':
                return cls._convert_to_html(
                    file_path, file_name, office_type, file_ext)
            elif conversion_format == 'pdf':
                return cls._convert_to_pdf(
                    file_path, file_name, office_type, file_ext)
            elif conversion_format == 'text':
                return cls._convert_to_text(
                    file_path, file_name, office_type, file_ext)
            elif conversion_format == 'image':
                return cls._convert_to_image(
                    file_path, file_name, office_type, file_ext)
            else:
                return cls._create_error_response(
                    file_name, f'不支持的转换格式: {conversion_format}')

        except Exception as e:
            logger.error(f'Office文档转换失败: {str(e)}', exc_info=True)
            return cls._create_error_response(
                file_name, f'Office文档转换失败: {str(e)}')
    ...
    def _process_office_file(
        ...
        try:
            ...
            if office_type == 'word':
                ...
                return cls._preview_powerpoint_file(
            ...
            return cls._create_error_response(
                file_name, f'Office文件处理失败: {str(e)}')
    ...
    def _convert_to_html(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                if office_type == 'word':
                    return cls._convert_word_to_html(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'excel':
                    return cls._convert_excel_to_html(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'powerpoint':
                    return cls._convert_powerpoint_to_html(
                        file_path, file_name, file_ext, temp_dir)
                else:
                    raise ValueError(f'不支持的Office类型: {office_type}')
                ...
                for table in doc.tables:
                    ...
                    for row in table.rows:
                        ...
                        for cell in row.cells:
                            html_content.append(
                                f'<td style="padding: 8px;">{cell.text}</td>')
                        html_content.append('</tr>')
                    ...
                    for row in range(1, max_row + 1):
                        ...
                        for col in range(1, max_col + 1):
                            ...
                            if cell_value is None:
                                cell_value = ''
                            html_content.append(
                                f'<td style="padding: 8px;">{cell_value}</td>')
                        html_content.append('</tr>')
                    html_content.append('</table>')
    ...
    def _convert_to_pdf(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                if office_type == 'word':
                    return cls._convert_word_to_pdf(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'excel':
                    return cls._convert_excel_to_pdf(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'powerpoint':
                    return cls._convert_powerpoint_to_pdf(
                        file_path, file_name, file_ext, temp_dir)
                else:
                    raise ValueError(f'不支持的Office类型: {office_type}')
    ...
    def _convert_word_to_pdf(
        ...
        try:
            ...
            pdf_path = os.path.join(
                temp_dir, f"{os.path.splitext(file_name)[0]}.pdf")
    ...
    def _convert_excel_to_pdf(
        ...
        try:
            ...
            html_result = cls._convert_excel_to_html(
                file_path, file_name, file_ext, temp_dir)
    ...
    def _convert_to_text(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                if office_type == 'word':
                    return cls._convert_word_to_text(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'excel':
                    return cls._convert_excel_to_text(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'powerpoint':
                    return cls._convert_powerpoint_to_text(
                        file_path, file_name, file_ext, temp_dir)
                else:
                    raise ValueError(f'不支持的Office类型: {office_type}')
    ...
    @classmethod
    def _convert_powerpoint_to_text(
            cls,
            file_path: str,
            file_name: str,
            file_ext: str,
        ...
        try:
            ...
            for para in doc.paragraphs:
                ...
                for run in para.runs:
                    ...
                    run_info = {
                        'text': run_text, 'bold': run.bold if hasattr(
                            run, 'bold') else False, 'italic': run.italic if hasattr(
                            run, 'italic') else False, 'underline': run.underline if hasattr(
                            run, 'underline') else False, 'font_size': None}

                    if run.font and hasattr(
                            run.font, 'size') and run.font.size:
            ...
            if doc.inline_shapes:
                for idx, shape in enumerate(doc.inline_shapes):
                    try:
                        ...
                        logger.warning(
                            f'处理Word图片 {idx+1} 失败: {str(img_error)}')
                        continue
    ...
    def _preview_excel_file(
        ...
        try:
            ...
            try:
                with tempfile.TemporaryDirectory() as temp_dir:
                    text_result = cls._convert_excel_to_text(
                        file_path, file_name, file_ext, temp_dir)
            ...
            for i, sheet_name in enumerate(sheet_names[:3]):  # 最多预览3个工作表
                try:
                    ...
                    for row_num, row in enumerate(
                            ws.iter_rows(max_row=10, values_only=True)):
                        try:
                            if row_num == 0:
                                # 第一行作为表头
                                headers = [
                            ...
                            logger.warning(
                                f'工作表 {sheet_name} 的行 {row_num+1} 处理失败: {str(row_error)}')
                            continue
                    ...
                    logger.warning(
                        f'工作表 {sheet_name} 处理失败: {str(sheet_error)}')
                    continue
                    ...
                    for row_num in range(min(10, ws.nrows)):
                        try:
                            ...
                            for col_num in range(
                                    min(10, ws.ncols)):  # 限制列数，避免处理过多列
                                try:
                                    cell_value = ws.cell_value(
                                        row_num, col_num)
                                    # 处理不同类型的单元格值
                                    if cell_value is None:
                                        cell_str = ''
                                    ...
                                    logger.warning(
                                        f'工作表 {sheet_name} 的单元格 ({row_num},{col_num}) 处理失败: {str(cell_error)}')
    ...
    def _preview_powerpoint_file(
        ...
        try:
            ...
            return cls._create_error_response(
                file_name, f'PowerPoint文档预览失败: {str(e)}')

    @classmethod
    def _preview_new_powerpoint_file(
            cls, file_path: str, file_name: str) -> dict:
            ...
            try:
                ...
                for i in range(slides_to_preview):
                    try:
                        ...
                        logger.warning(
                            f'访问幻灯片 {i+1} 失败: {str(slide_access_error)}')
                        break
                    ...
                    try:
                        if slide.shapes.title and slide.shapes.title.text.strip():
                            slide_content['title'] = TextCleaner.clean_text(
                        ...
                        for shape_idx, shape in enumerate(slide.shapes):
                            try:
                                ...
                                shape_info = {
                                    'type': shape.shape_type.name if hasattr(
                                        shape.shape_type,
                                        'name') else str(
                                        shape.shape_type),
                                    'name': shape.name,
                                ...
                                if hasattr(
                                        shape, 'text') and shape.text.strip():
                                    shape_info['text'] = TextCleaner.clean_text(
                                        shape.text.strip())

                                    if hasattr(shape, 'text_frame'):
                                        tf = shape.text_frame
                                        paragraphs = []
                                        for p_idx, para in enumerate(
                                                tf.paragraphs):
                                            para_info = {
                                                ...
                                                'font_style': PPTStyleExtractor.extract_font_style(
                                                    para.font) if para.font else {}}
                                    ...
                                    if hasattr(
                                            shape, 'shape_type') and shape.shape_type == MSO_SHAPE:
                                        shape.shape_type
                                        shape_name = shape.name.lower() if shape.name else ''

                                        if 'picture' in shape_name or hasattr(
                                                shape, 'image'):
                                            try:
                                                image_info = {
                                                    'type': 'image',
                                                ...
                                                if hasattr(
                                                        shape, 'width') and shape.width:
                                                    image_info['width'] = shape.width.pt
                                                if hasattr(
                                                        shape, 'height') and shape.height:
                                                    image_info['height'] = shape.height.pt

                                                if hasattr(
                                                        shape, 'alt_text') and shape.alt_text:
                                                    image_info['description'] = shape.alt_text

                                                if hasattr(
                                                        shape, '_inline') and shape._inline:
                                                    try:
                                                        rId = shape._inline.docPr.get(
                                                            'id')
                                                        if rId:
                                                            rId_str = str(rId)
                                                            for img_name, img_data in image_map.items():
                                                                if img_data.get(
                                                                        'rId') == rId_str or img_name:
                                                                    image_info['data'] = img_data.get(
                                                                        'data')
                                                                    image_info['is_placeholder'] = False
                                                                    image_info['format'] = img_data.get(
                                                                        'format')
                                                                    break
                                                    except BaseException:
                                                        pass
                                ...
                                logger.warning(
                                    f'幻灯片 {i+1} 中形状 {shape_idx} 处理失败: {str(shape_error)}')
                                continue

                        if not slide_content['title'] and slide_content['content_blocks']:
                            first_text = slide_content['content_blocks'][0].get(
                                'text', '') if slide_content['content_blocks'] else ''
                            if first_text:
                                slide_content['title'] = first_text[:50] + \
                                    ('...' if len(first_text) > 50 else '')
    ...
    @classmethod
    def _preview_old_powerpoint_file(
            cls, file_path: str, file_name: str) -> dict:
            ...
            try:
                ...
                for page_num in range(min(10, doc_len)):  # 最多预览10页
                    try:
                        ...
                        logger.warning(
                            f'PyMuPDF处理幻灯片 {page_num+1} 失败: {str(page_error)}')
                        continue
    ...
    def _convert_to_image(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                if office_type == 'word':
                    return cls._convert_word_to_image(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'excel':
                    return cls._convert_excel_to_image(
                        file_path, file_name, file_ext, temp_dir)
                elif office_type == 'powerpoint':
                    return cls._convert_powerpoint_to_image(
                        file_path, file_name, file_ext, temp_dir)
                else:
                    raise ValueError(f'不支持的Office类型: {office_type}')
    ...
    @classmethod
    def _convert_powerpoint_to_image(
            cls,
            file_path: str,
            file_name: str,
            file_ext: str,
        ...
        try:
            ...
            for page_num in range(min(10, len(doc))):  # 最多处理10张幻灯片
                ...
                image_path = os.path.join(
                    temp_dir, f"slide_{page_num + 1}.png")
                pix.save(image_path)
