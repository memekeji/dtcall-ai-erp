...
class DiskFolderForm(forms.ModelForm):
    class Meta:
        model = DiskFolder
        fields = ['name', 'parent', 'department', 'is_public', 'permission_level']
        widgets = {
            ...
            'is_public': forms.CheckboxInput(attrs={'lay-skin': 'switch', 'lay-text': '是|否'}),
            'permission_level': forms.Select(attrs={'class': 'layui-input'}, choices=DiskFolder.PERMISSION_LEVELS),
        }
...
class DiskShareForm(forms.ModelForm):
    class Meta:
        model = DiskShare
        fields = ['share_type', 'password', 'expire_time', 'permission_type', 'allow_download', 'access_limit', 'download_limit']
        widgets = {
            ...
            'permission_type': forms.Select(attrs={'class': 'layui-input'}, choices=DiskShare.PERMISSION_TYPES),
            'allow_download': forms.CheckboxInput(attrs={'lay-skin': 'switch', 'lay-text': '允许|禁止'}),
            'access_limit': forms.NumberInput(attrs={
                'class': 'layui-input', 
...
class FileMoveForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user:
            self.fields['target_folder_id'].widget = forms.Select(attrs={'class': 'layui-input'})
