...
class DataIntegrationService:
    ...
    @staticmethod
    @transaction.atomic
    def create_contract_with_project(
            customer_id,
            contract_data,
            project_data=None):
        """
        ...
        try:
            ...
            if project_data:
                ...
                logger.info(
                    f"创建合同{contract.contract_number}并关联项目{project.name}")
    ...
    def update_contract_status(contract_id, new_status):
        ...
        try:
            ...
            if new_status in project_status_mapping:
                ...
                logger.info(
                    f"合同{contract.contract_number}状态从{old_status}更新为{new_status}，同步更新关联项目")
    ...
    def update_project_budget(project_id, new_budget):
        ...
        try:
            project = Project.objects.select_related(
                'contract').get(id=project_id)

            # 如果项目关联了合同，验证预算是否合理
            if project.contract and new_budget > project.contract.amount:
                logger.warning(
                    f"项目{project.name}预算{new_budget}超过合同金额{project.contract.amount}")
    ...
    def get_contract_related_data(contract_id):
        ...
        try:
            ...
            finance_records = OrderFinanceRecord.objects.filter(
            ...
            total_paid_amount = sum(
                record.paid_amount for record in finance_records)
    ...
    def reconcile_financial_data():
        ...
        try:
            ...
            orders_without_finance = CustomerOrder.objects.filter(
                ~Q(id__in=OrderFinanceRecord.objects.values_list(
                    'order_id', flat=True))
            )
