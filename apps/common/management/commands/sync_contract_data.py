    ...
    def handle(self, *args, **options):
        ...
        if contract_id:
            ...
            try:
                ...
                self.stdout.write(
                    f'开始同步合同: {contract.contract_number} ({contract.name})')
                ...
                with transaction.atomic():
                    ...
                    sync_contract_to_project(
                        sender=CustomerContract, instance=contract, created=False, **{})
                ...
                self.stdout.write(
                    self.style.ERROR(f'合同 {contract.id} 同步失败: {str(e)}'))
            ...
            for contract in contracts:
                try:
                    self.stdout.write(
                        f'同步合同: {contract.contract_number} ({contract.name})')

                    # 强制修复合同可见性
                    if hasattr(contract,
                               'delete_time') and contract.delete_time:
                        contract.delete_time = 0
                        contract.save(update_fields=['delete_time'])
                        self.stdout.write(f'已修复合同 {contract.id} 的可见性')
                    ...
                    logger.error(traceback.format_exc())
                    self.stdout.write(
                        self.style.ERROR(f'同步合同 {contract.id} 失败: {str(e)}'))
                    failed_count += 1
                    failed_contracts.append((contract.id, str(e)))

            self.stdout.write(self.style.SUCCESS(
                f'合同同步完成: 成功 {success_count}, 失败 {failed_count}'))
            ...
            self.stdout.write(self.style.WARNING(
                '请指定 --contract-id 或 --all 参数'))
            self.stdout.write('示例:')
            self.stdout.write(
                '  python manage.py sync_contract_data --contract-id 1')
            self.stdout.write('  python manage.py sync_contract_data --all')
            self.stdout.write(
                '  python manage.py sync_contract_data --all --signed-only')
    ...
    def _verify_contract_relations(self, contract):
        ...
        try:
            ...
            project_exists = Project.objects.filter(
                contract_id=contract.id).exists()
            if project_exists:
            ...
            if order_exists:
                ...
                # 检查财务记录
                finance_exists = OrderFinanceRecord.objects.filter(
                    order=order).exists()
                verification_msg.append(
                    f'✓ 财务记录' if finance_exists else f'✗ 财务记录')

                # 检查开票申请
                invoice_request_exists = InvoiceRequest.objects.filter(
                    order=order).exists()
