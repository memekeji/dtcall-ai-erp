...
class DataSyncService:
    ...
    def sync_user_profile(cls, user_id=None):
        ...
        try:
            ...
            users = Admin.objects.filter(id=user_id) if user_id else Admin.objects.all()
    ...
    def sync_customer_business(cls, customer_id=None):
        ...
        try:
            ...
            customers = Customer.objects.filter(id=customer_id) if customer_id else Customer.objects.all()
            
            for customer in customers:
                # 计算合同统计数据
                if 'CustomerContract' in globals():
                    contracts = CustomerContract.objects.filter(customer=customer)
                    total_contracts = contracts.count()
                    active_contracts = contracts.filter(status__in=['signed', 'executing']).count()
                    ...
                    if 'CustomerOrder' in globals():
                        ...
                        total_order_amount = orders.aggregate(Sum('amount'))['amount__sum'] or 0
                    
                    # 计算财务统计数据
                    if 'Invoice' in globals():
                        invoices = Invoice.objects.filter(customer_id=customer.id)
                        total_invoice_amount = invoices.aggregate(Sum('amount'))['amount__sum'] or 0
                        paid_invoice_amount = invoices.filter(status='paid').aggregate(Sum('amount'))['amount__sum'] or 0
                    
                    # 计算项目统计数据
                    if 'Project' in globals():
                        projects = Project.objects.filter(customer_id=customer.id)
                        total_projects = projects.count()
                        completed_projects = projects.filter(status='completed').count()
                    
                    logger.info(f"客户 {customer.name} 同步完成: 合同总数={total_contracts}, 总金额={total_amount}")
    ...
    def sync_customer_contract_to_project(cls, contract_id):
        ...
        try:
            ...
            # 检查是否已存在关联项目
            existing_project = Project.objects.filter(contract_id=contract_id).first()
            ...
            if not existing_project:
                ...
                # 确保合同编号已设置
                if hasattr(existing_project, 'contract_number') and not existing_project.contract_number:
                    existing_project.contract_number = contract.contract_number
                
                existing_project.save()
                logger.info(f"更新了合同 {contract_id} 的关联项目 {existing_project.id}，状态更新为: {project_status}")
                
            # 触发项目相关数据的同步
            try:
                if 'DataSyncService' in globals() and hasattr(DataSyncService, 'sync_customer_business'):
                    DataSyncService.sync_customer_business(contract.customer.id)
    ...
    def sync_customer_order_to_finance(cls, order_id):
        ...
        try:
            # 检查必要的模型是否可用
            if 'CustomerOrder' not in globals() or 'Invoice' not in globals() or 'OrderFinanceRecord' not in globals():
            ...
            if hasattr(order, 'delete_time') and order.delete_time > 0:
                ...
                # 考虑将关联的财务记录标记为无效或取消
                try:
                    finance_record = OrderFinanceRecord.objects.filter(order_id=order_id).first()
                    if finance_record and hasattr(finance_record, 'status') and finance_record.status != 'cancelled':
            ...
            if not finance_record:
                # 创建新的财务记录
                finance_record = OrderFinanceRecord.objects.create(**finance_data)
                ...
                finance_record.save()
                logger.info(f"更新了订单 {order_id} 的关联财务记录 {finance_record.id}，状态更新为: {payment_status}")
    ...
    def sync_contract_module_to_customer_contract(cls, contract_module_id):
        ...
        try:
            # 检查必要的模型是否可用
            if 'ContractModule' not in globals() or 'CustomerContract' not in globals() or 'Customer' not in globals():
            ...
            if customer_contract:
                ...
                if contract_module.sign_time:
                    customer_contract.sign_date = datetime.fromtimestamp(contract_module.sign_time).date()
                if contract_module.end_time:
                    customer_contract.end_date = datetime.fromtimestamp(contract_module.end_time).date()
                ...
                logger.info(f"同步合同管理模块合同 {contract_module_id} 到客户合同 {customer_contract.id}")
            else:
                # 创建新的客户合同
                try:
                    customer = Customer.objects.get(id=contract_module.customer_id)
                    # 准备创建客户合同的数据
                    contract_data = {
                        'customer': customer,
                    ...
                    # 设置时间和状态
                    if contract_module.sign_time:
                        contract_data['sign_date'] = datetime.fromtimestamp(contract_module.sign_time).date()
                    if contract_module.end_time:
                        contract_data['end_date'] = datetime.fromtimestamp(contract_module.end_time).date()
                    ...
                    contract_data['status'] = status_map.get(contract_module.status, 'draft')
                    
                    # 创建客户合同
                    customer_contract = CustomerContract.objects.create(**contract_data)
                    logger.info(f"创建客户合同 {customer_contract.id} 对应合同管理模块合同 {contract_module_id}")
                    
                    # 如果是已签署状态，同步到项目
                    if contract_data['status'] == 'signed':
                        try:
                            DataSyncService.sync_customer_contract_to_project(customer_contract.id)
                            logger.info(f"已自动将新建客户合同 {customer_contract.id} 同步到项目模块")
                        except Exception as e:
                            logger.warning(f"同步新建客户合同到项目失败: {str(e)}")
                            
                except Customer.DoesNotExist:
                    logger.error(f"客户 {contract_module.customer_id} 不存在，无法创建客户合同")
                except Exception as e:
        ...
        try:
            ...
            for issue in issues:
                ...
                if issue_type == 'missing_finance_records' and 'affected_ids' in issue:
                    result = cls._fix_missing_finance_records(issue['affected_ids'])
                ...
                # 4. 修复项目状态与合同状态不一致
                    ...
                    result = cls._fix_inconsistent_project_statuses(issue['details'])
                    fixed_count += result['fixed']
                    error_count += result['errors']
                    skipped_count += result['skipped']
                    results.append(result)
                ...
                # 5. 修复项目预算与合同金额不一致
                    ...
                    result = cls._fix_inconsistent_project_budgets(issue['details'])
                    fixed_count += result['fixed']
                    error_count += result['errors']
                    skipped_count += result['skipped']
                    results.append(result)
                ...
                # 6. 修复订单金额与财务记录金额不一致
                    ...
                    result = cls._fix_inconsistent_order_finance_amounts(issue['details'])
                    fixed_count += result['fixed']
                    error_count += result['errors']
                    skipped_count += result['skipped']
                    results.append(result)
                ...
                # 7. 修复缺少双向合同映射
                    ...
                    result = cls._fix_missing_contract_mappings(issue['details'])
                    fixed_count += result['fixed']
                    error_count += result['errors']
                    skipped_count += result['skipped']
                    results.append(result)
                ...
                # 8. 修复财务与发票状态不一致
                    ...
                    result = cls._fix_finance_invoice_statuses(issue['affected_ids'])
                    fixed_count += result['fixed']
                    error_count += result['errors']
                    skipped_count += result['skipped']
                    results.append(result)
            
            logger.info(f"自动修复完成，成功修复 {fixed_count} 个问题，失败 {error_count} 个，跳过 {skipped_count} 个")
        ...
        for detail in details[:50]:
            try:
                ...
                if 'contract_module_id' in detail:
                    DataSyncService.sync_contract_module_to_customer_contract(detail['contract_module_id'])
                    fixed += 1
                    logger.info(f"已修复合同 {detail.get('contract_id')} 与合同管理模块 {detail['contract_module_id']} 的数据一致性")
                else:
                    skipped += 1
    ...
    def _fix_inconsistent_project_statuses(cls, details):
        ...
        for detail in details[:50]:
            try:
                ...
                contract = CustomerContract.objects.get(id=detail['contract_id'])
                ...
                if new_status and new_status != project.status:
                    ...
                    project.save()
                    fixed += 1
                    logger.info(f"已修复项目 {project.id} 状态，从 {detail['project_status']} 改为 {new_status}")
                else:
                    skipped += 1
                ...
                errors += 1
                logger.error(f"修复项目 {detail.get('project_id')} 状态不一致失败: {str(e)}")
        ...
        for detail in details[:50]:
            try:
                ...
                if detail.get('contract_amount') != project.budget:
                    ...
                    project.save()
                    fixed += 1
                    logger.info(f"已修复项目 {project.id} 预算，更新为 {detail.get('contract_amount')}")
                else:
                    skipped += 1
                ...
                errors += 1
                logger.error(f"修复项目 {detail.get('project_id')} 预算不一致失败: {str(e)}")
    ...
    def _fix_inconsistent_order_finance_amounts(cls, details):
        ...
        for detail in details[:50]:
            try:
                ...
                finance_record = OrderFinanceRecord.objects.get(id=detail['finance_record_id'])
                ...
                if detail.get('order_amount') != finance_record.amount:
                    ...
                    finance_record.save()
                    fixed += 1
                    logger.info(f"已修复财务记录 {finance_record.id} 金额，更新为 {detail.get('order_amount')}")
                else:
                    skipped += 1
            except OrderFinanceRecord.DoesNotExist:
                skipped += 1
                logger.warning(f"财务记录 {detail.get('finance_record_id')} 不存在，跳过修复")
            except Exception as e:
                errors += 1
                logger.error(f"修复财务记录 {detail.get('finance_record_id')} 金额不一致失败: {str(e)}")
    ...
    def _fix_missing_contract_mappings(cls, details):
        ...
        for detail in details[:50]:
            try:
                ...
                # 检查修复动作
                if detail.get('fix_action') == 'create_contract_module' and 'contract_id' in detail:
                    # 从客户合同创建合同管理模块记录
                    customer_contract = CustomerContract.objects.get(id=detail['contract_id'])
                    
                    # 检查是否已存在
                    if not ContractModule.objects.filter(code=customer_contract.contract_number).exists():
                        contract_module = ContractModule(
                            ...
                            sign_time=int(customer_contract.sign_date.timestamp()) if customer_contract.sign_date else int(time.time()),
                            create_time=int(time.time()),
                            update_time=int(time.time())
                        )
                        contract_module.save()
                        fixed += 1
                        logger.info(f"已为客户合同 {customer_contract.id} 创建关联的合同管理模块记录")
                    else:
                        skipped += 1
                elif detail.get('fix_action') == 'create_customer_contract' and 'contract_module_id' in detail:
                    # 从合同管理模块创建客户合同记录
                    contract_module = ContractModule.objects.get(id=detail['contract_module_id'])
                    
                    # 检查是否已存在
                    if not CustomerContract.objects.filter(contract_number=contract_module.contract_number).exists():
                        # 状态映射
                        status_map = {
                            1: 'signed',
                            2: 'executing',
                        ...
                        customer_contract = CustomerContract(
                            ...
                            name=contract_module.name,
                            amount=contract_module.cost,
                            status=status_map.get(contract_module.status, 'draft'),
                            sign_date=datetime.fromtimestamp(contract_module.sign_time).date() if contract_module.sign_time else None,
                            create_time=int(time.time()),
                            update_time=int(time.time())
                        )
                        customer_contract.save()
                        fixed += 1
                        logger.info(f"已为合同管理模块 {contract_module.id} 创建关联的客户合同记录")
                    else:
                        skipped += 1
                else:
                    skipped += 1
                    logger.warning(f"跳过合同映射修复，未知的修复动作: {detail.get('fix_action')}")
        ...
        try:
            ...
            def sync_customer_business_on_customer_contract_save(sender, instance, created, **kwargs):
                ...
                if created or instance.status == 'signed':
                    DataSyncService.sync_customer_contract_to_project(instance.id)
            signal_handlers.append('sync_customer_business_on_customer_contract_save')
        except Exception as e:
    ...
    if 'ContractModule' in globals():
        try:
            ...
            def sync_contract_module_to_customer(sender, instance, **kwargs):
                """合同管理模块合同保存时同步到客户模块"""
                # 尝试同步到客户合同模块
                DataSyncService.sync_contract_module_to_customer_contract(instance.id)
                # 更新相关客户的业务数据
        ...
        try:
            ...
            def sync_customer_business_on_payment_save(sender, instance, **kwargs):
                ...
                # 如果付款关联了合同，通过合同找到客户
                    ...
                    try:
                        contract = CustomerContract.objects.get(id=instance.contract_id)
                        DataSyncService.sync_customer_business(contract.customer.id)
...
class DataQualityMonitor:
    ...
    def check_data_consistency(cls):
        ...
        try:
            ...
            if 'Admin' in globals() and 'EmployeeFile' in globals():
                users_without_profile = Admin.objects.filter(employee_file__isnull=True)
            ...
            if 'CustomerContract' in globals() and 'Project' in globals():
                ...
                contracts_without_project = CustomerContract.objects.filter(
                    ...
                    id__in=Project.objects.values_list('contract_id', flat=True)
                )
            ...
            if 'CustomerOrder' in globals() and 'OrderFinanceRecord' in globals():
                ...
                orders_without_finance = CustomerOrder.objects.filter(
                    ...
                    id__in=OrderFinanceRecord.objects.values_list('order_id', flat=True)
                )
            ...
            if 'CustomerContract' in globals() and 'ContractModule' in globals():
                ...
                customer_contracts = CustomerContract.objects.filter(delete_time=0)[:200]
                for customer_contract in customer_contracts:
                    try:
                        ...
                        if contract_module:
                            # 检查金额是否一致
                            if abs(float(customer_contract.amount) - float(contract_module.cost)) > 0.01:
                            ...
                            if status_map.get(customer_contract.status) != contract_module.status:
                                inconsistent_contracts.append({
                                    'contract_id': customer_contract.id,
                                    'contract_module_id': contract_module.id,
                            ...
                            if customer_contract.sign_date and contract_module.sign_time:
                                contract_date = datetime.fromtimestamp(contract_module.sign_time).date()
                        ...
                        logger.error(f"检查合同 {customer_contract.id} 一致性失败: {str(e)}")
                
                # 4.2 双向检查 - 合同管理模块到客户合同
                contract_modules = ContractModule.objects.filter(delete_time=0)[:200]
                for contract_module in contract_modules:
                    try:
                        ...
                        logger.error(f"检查合同管理模块 {contract_module.id} 一致性失败: {str(e)}")
    ...
    def _check_project_contract_status_consistency(cls, issues):
        ...
        try:
            ...
            for project in projects_with_contract:
                try:
                    contract = CustomerContract.objects.get(id=project.contract_id)
                    # 检查合同状态对应的项目状态是否匹配
                    allowed_statuses = contract_project_status_map.get(contract.status, [])
        ...
        try:
            ...
            for project in projects_with_contract:
                try:
                    contract = CustomerContract.objects.get(id=project.contract_id)
                    # 检查预算与合同金额是否偏差过大
                    if abs(float(project.budget) - float(contract.amount)) > 0.01:
    ...
    def _check_order_finance_amount_consistency(cls, issues):
        ...
        try:
            ...
            for order in orders_with_finance:
                try:
                    finance_record = OrderFinanceRecord.objects.get(order_id=order.id)
                    # 检查金额是否一致
                    if abs(float(order.amount) - float(finance_record.amount)) > 0.01:
    ...
    def _check_finance_invoice_consistency(cls, issues):
        ...
        try:
            ...
            for invoice in invoices:
                try:
                    finance_record = OrderFinanceRecord.objects.get(order_id=invoice.order_id)
                    if abs(float(invoice.amount) - float(finance_record.amount)) > 0.01:
        ...
        if not data:
            try:
                ...
                if 'Payment' in globals():
                    ...
                    paid_amount = Payment.objects.filter(contract_id__in=contract_ids, status='paid').aggregate(
                        Sum('amount')
                    )['amount__sum'] or 0
        ...
        if not data:
            try:
                ...
                if 'RewardPunishment' in globals():
                    data['total_rewards'] = RewardPunishment.objects.filter(employee=employee, type='reward').count()
                    data['total_punishments'] = RewardPunishment.objects.filter(employee=employee, type='punishment').count()
