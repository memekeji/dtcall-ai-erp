...
# 导入所有需要的模型
from apps.customer.models import CustomerContract, CustomerOrder, Customer
...
class DataIntegrationService:
    ...
    def create_contract_with_project(self, contract):
        ...
        try:
            ...
            logger.info(f"为合同 {contract.id} 创建项目 {project.id}，项目代码: {project_code}")
            return project
    ...
    def create_order_with_finance(self, contract):
        ...
        try:
            ...
            try:
                # 先尝试查找现有财务记录
                finance_record = OrderFinanceRecord.objects.filter(order=order).first()
                if finance_record:
                    # 更新现有记录
        ...
        try:
            ...
            if customer:
                invoice = Invoice.objects.create(
                    ...
                    project=Project.objects.filter(contract_id=order.contract_id).first(),
                    amount=order.amount,
                    did=0,
                    ...
                    invoice_tax=customer.tax_number if hasattr(customer, 'tax_number') else '',
                    enter_amount=0,
                    enter_status=0,
                    check_status=0,
                    create_time=int(order.create_time.timestamp()) if order.create_time else int(timezone.now().timestamp())
                )
...
def sync_contract_to_project(sender, instance, created, **kwargs):
    ...
    """
    try:
        logger.info(f"触发合同同步信号: ID={instance.id}, created={created}, status={instance.status}")
        ...
        if instance.status == 'signed':
            ...
            # 检查是否已有相关记录，避免重复创建
            project_exists = Project.objects.filter(contract_id=instance.id).exists()
            order_exists = CustomerOrder.objects.filter(contract_id=instance.id).exists()
            
            logger.info(f"合同 {instance.id} 检查: 项目存在={project_exists}, 订单存在={order_exists}")
            
            # 创建项目
            if not project_exists:
                project = data_integration_service.create_contract_with_project(instance)
            else:
            ...
            # 创建订单和财务记录
            if not order_exists:
                order, finance_record = data_integration_service.create_order_with_finance(instance)
                ...
                # 检查是否有待开票记录
                if not InvoiceRequest.objects.filter(order=order).exists() and not Invoice.objects.filter(contract_id=instance.id).exists():
                    data_integration_service.create_invoice_request(order)
        else:
            # 合同更新时，同步更新关联项目状态
            if hasattr(data_integration_service, 'update_contract_status'):
                data_integration_service.update_contract_status(instance.id, instance.status)
            else:
        ...
        if created:
            ...
            try:
                  ...
                  if finance_record.total_amount != instance.amount:
                      ...
                      finance_record.save()
                      logger.info(f"订单{instance.order_number}金额变更，财务记录从{old_amount}更新为{instance.amount}")
    ...
    try:
        ...
        if hasattr(instance, 'invoice_request') and instance.invoice_request:
            ...
            if order.invoice_request_status == 'approved':
                ...
                order.save()
                logger.info(f"发票{instance.code}已创建，更新订单{order.order_number}开票状态")
...
def prevent_delete_used_contract(sender, instance, **kwargs):
    ...
    if Project.objects.filter(contract=instance).exists():
        ...
        raise ValueError(f"无法删除合同 {instance.contract_number}，该合同已关联 {projects_count} 个项目")
    ...
    if CustomerOrder.objects.filter(contract=instance).exists():
        ...
        raise ValueError(f"无法删除合同 {instance.contract_number}，该合同已关联 {orders_count} 个订单")
    
    # 检查是否有发票关联此合同
    if Invoice.objects.filter(contract=instance).exists():
        invoices_count = Invoice.objects.filter(contract=instance).count()
        raise ValueError(f"无法删除合同 {instance.contract_number}，该合同已关联 {invoices_count} 张发票")
    ...
    try:
        if instance.contract and instance.budget:
            ...
            if budget > contract_amount:
                logger.warning(f"项目{instance.name}预算{budget}超过合同金额{contract_amount}")
