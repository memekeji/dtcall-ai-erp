...
def timestamp_to_date(timestamp: Optional[int], format_str: str = '%Y-%m-%d %H:%M') -> str:
    """将时间戳转换为日期字符串
...
def build_success_response(data: Any = None, message: str = '', count: int = 0) -> JsonResponse:
    """构建成功响应
...
def get_date_range_filter(field_name: str, date_range_str: str):
    ...
    try:
        start_str, end_str = date_range_str.split(' - ')
        start_timestamp = int(datetime.strptime(start_str.strip(), '%Y-%m-%d').timestamp())
        end_timestamp = int(datetime.strptime(end_str.strip() + ' 23:59:59', '%Y-%m-%d %H:%M:%S').timestamp())
        
        return Q(**{f'{field_name}__gte': start_timestamp}) & Q(**{f'{field_name}__lte': end_timestamp})
    except (ValueError, TypeError):
        return Q()
...
class PaginationHelper:
    ...
    def __init__(self, request, default_page: int = 1, default_limit: int = 20):
        """初始化
...
class StatusCodeMapper:
    ...
    @classmethod
    def get_display(cls, name: str, status_code: int, default: str = '未知') -> str:
        """获取状态描述
...
def batch_process(items: List, processor: callable, batch_size: int = 50) -> Dict:
    """批量处理数据
