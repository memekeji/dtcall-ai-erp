...
def dashboard(request):
    ...
    # 统计数据
    stats = {
        'total_tasks': PersonalTask.objects.filter(
        ...
        'pending_tasks': PersonalTask.objects.filter(
                ...
                status__in=[
                    'todo',
                    'in_progress']).count(),
        'total_notes': PersonalNote.objects.filter(
                        user=request.user).count(),
    }
...
def minutes_form(request, pk=None):
        ...
        if request.POST.get('generate_ai_minutes') == 'true':
            try:
                ...
                reservation_id = request.POST.get(
                    'reservation_id') or request.GET.get('reservation_id')
                ...
                if not meeting_id and reservation_id:
                    try:
                        ...
                        # 获取预约记录
                        reservation = MeetingReservation.objects.get(
                            pk=reservation_id)
                        ...
                        if not meeting_record:
                            ...
                            participants_list = list(
                                reservation.attendees.all())
                            if participants_list:
                                meeting_record.participants.set(
                                    participants_list)

                            import logging
                            logger = logging.getLogger(__name__)
                            logger.info(
                                f"AI生成时基于预约ID {reservation_id} 自动创建了会议记录 {meeting_record.id}")
                        ...
                        logger = logging.getLogger(__name__)
                        logger.info(
                            f"从预约ID {reservation_id} 关联到会议记录ID {meeting_id}")
                    except Exception as e:
                        import logging
                        logger = logging.getLogger(__name__)
                        logger.error(f"基于预约ID创建或获取会议记录失败: {str(e)}")
                        return JsonResponse(
                            {'code': 1, 'msg': '无法基于预约信息创建会议记录，请手动选择会议记录'})

                # 如果仍然没有会议ID，返回错误
                if not meeting_id:
                    return JsonResponse(
                        {'code': 1, 'msg': '会议ID不能为空，请先选择一个会议记录'})
                ...
                try:
                    ...
                    if not (meeting_record.host == request.user or
                        ...
                        return JsonResponse(
                            {'code': 1, 'msg': '您没有权限为此会议生成纪要'})
                except MeetingRecord.DoesNotExist:
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.error(
                        f"会议记录不存在：会议ID {meeting_id} 未找到对应的MeetingRecord")

                    # 检查是否可能是reservation_id，如果是，给出更友好的错误提示
                    if reservation_id and meeting_id == reservation_id:
                        return JsonResponse(
                            {'code': 1, 'msg': '未找到对应的会议记录。请注意：会议室预约ID和会议记录ID是不同的概念，请先创建会议记录，或者从会议记录页面进入纪要编辑'})
                    else:
                        # 检查是否有对应的会议室预约，提供更详细的提示
                        try:
                            from apps.system.models import MeetingReservation
                            reservation = MeetingReservation.objects.filter(
                                pk=meeting_id).first()
                            if reservation:
                                return JsonResponse(
                                    {'code': 1, 'msg': '您提供的ID似乎是会议室预约ID，而不是会议记录ID。请先基于此预约创建会议记录，然后再生成纪要'})
                        except Exception:
                            pass

                        return JsonResponse(
                            {'code': 1, 'msg': f'会议记录不存在：会议ID {meeting_id} 未找到，请确认您已选择了正确的会议记录'})
                ...
                if audio_file:
                    ...
                    if ext not in allowed_extensions:
                        return JsonResponse(
                            {'code': 1, 'msg': '不支持的音频格式，请上传mp3、wav、flac、ogg或aac格式'})
                    ...
                    upload_dir = os.path.join(
                        settings.MEDIA_ROOT, 'meeting_recordings')
                    ...
                    for attachment in meeting_record.attachments.split(','):
                        if any(attachment.lower().endswith(ext)
                ...
                if not audio_file_path:
                    return JsonResponse(
                        {'code': 1, 'msg': '请上传音频文件或选择已有音频的会议记录'})
        ...
        if form.is_valid():
            try:
                ...
                meeting_id = request.POST.get(
                    'meeting_id') or request.GET.get('meeting_id')
                if meeting_id:
                    try:
                        from apps.oa.models import MeetingRecord
                        meeting_record = MeetingRecord.objects.get(
                            pk=meeting_id)
        ...
        if not minute:
            ...
            # 检查是否有预约ID参数，有则填充表单
                ...
                try:
                    ...
                    from apps.oa.models import MeetingRecord
                    meeting_record = MeetingRecord.objects.filter(
                        title=reservation.title, meeting_date=reservation.start_time).first()
                    ...
                    if not meeting_record:
                        ...
                        logger = logging.getLogger(__name__)
                        logger.info(
                            f"基于预约ID {reservation_id} 自动创建了会议记录 {meeting_record.id}")
    ...
    user_meetings = MeetingRecord.objects.filter(
        Q(host=request.user) | Q(recorder=request.user) | Q(
