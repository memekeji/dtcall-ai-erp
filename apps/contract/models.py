...
class ContractCategory(models.Model):
    ...
    template_path = models.CharField(
        max_length=200, blank=True, verbose_name='合同模板路径')
    ...
    def save(self, *args, **kwargs):
        if not self.code:
            ...
            while ContractCategory.objects.filter(
                    code=temp_code).exclude(
                    pk=self.pk).exists():
                suffix = f"_{counter}"
...
class Service(models.Model):
    ...
    duration = models.IntegerField(
        null=True, blank=True, verbose_name='服务周期(天)')
...
class Supplier(models.Model):
    ...
    bank_account = models.CharField(
        max_length=100, blank=True, verbose_name='银行账号')
    bank_name = models.CharField(
        max_length=100,
        blank=True,
        verbose_name='开户银行')
    credit_level = models.CharField(
        max_length=20, blank=True, verbose_name='信用等级')
...
class PurchaseItem(models.Model):
    ...
    specification = models.CharField(
        max_length=500, blank=True, verbose_name='规格型号')
    unit = models.CharField(max_length=20, blank=True, verbose_name='计量单位')
...
class Contract(models.Model):
    ...
    contact_name = models.CharField(
        max_length=255, default='', verbose_name='客户代表')
    contact_mobile = models.CharField(
        max_length=255, default='', verbose_name='客户电话')
    contact_address = models.CharField(
        max_length=255, default='', verbose_name='客户地址')
    ...
    # AI 智能分析扩展字段
    ai_risk_level = models.CharField(max_length=20, default='unknown', verbose_name='AI风险评级(高/中/低)')
    ai_risk_points = models.JSONField(default=list, blank=True, verbose_name='AI识别风险点')
    ai_key_terms = models.JSONField(default=list, blank=True, verbose_name='AI提取关键条款')
    ...
    check_uids = models.CharField(
        max_length=500,
        default='',
        verbose_name='当前审批人ID')
    check_last_uid = models.CharField(
        max_length=500, default='', verbose_name='上一审批人')
...
class Purchase(models.Model):
    ...
    admin = models.ForeignKey(
        ...
        verbose_name='管理员')
    sign_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name='purchase_sign',
        verbose_name='签订人')
    prepared_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name='purchase_prepared',
        verbose_name='编制人')
    keeper_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
    ...
    check_uids = models.CharField(
        max_length=255,
        blank=True,
        verbose_name='待审核人员ID')
    check_history_uids = models.CharField(
        max_length=255, blank=True, verbose_name='已审核人员ID')
    file_ids = models.CharField(
        max_length=255,
        blank=True,
        verbose_name='附件ID')
    ...
    archive_time = models.DateTimeField(
        null=True, blank=True, verbose_name='归档时间')
    stop_time = models.DateTimeField(
        null=True, blank=True, verbose_name='终止时间')
    void_time = models.DateTimeField(
        null=True, blank=True, verbose_name='作废时间')
    archive_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name='purchase_archive',
        verbose_name='归档人')
    stop_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        related_name='purchase_stop',
        verbose_name='终止人')
    void_uid = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
