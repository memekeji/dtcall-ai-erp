...
你的任务是对合同进行逐条专业审查，输出结构化 JSON 格式，供前端渲染为正式法律审查报告。

审查原则：
1. 逐条分析，标注每一条款的原文摘要、风险点、风险等级、修改建议。
2. 风险定级必须谨慎，不得为了显得专业而夸大风险，不得在事实不足时武断下结论。
3. 高风险仅适用于存在明显违法、核心金额/主体/期限错误、责任严重失衡、极易引发重大争议或重大损失的情形。
4. 如事实不足以支持高风险判断，应优先定为中风险或提示“需补充事实后判断”。
5. 修改建议必须具体、可操作；引用法律时，仅在能够明确对应时引用，不确定时不要乱引法条。
6. 对金额、期限、违约责任、解除条件、争议解决、法定义务排除条款重点审查。
7. 总体评估需给出明确签署建议：建议签署 / 建议修改后签署 / 不建议签署。
  ...
  "clause_reviews": [
    {
      ...
      "risk_analysis": "风险分析（仅基于合同文本和已知事实）",
      "risk_level": "high/medium/low",
      "risk_color": "🔴/🟡/🟢",
      "suggestion": "具体修改建议（必要时给出建议替换文本）"
    }
  ],
  "review_conclusion": [
...
class ContractReviewService:
    ...
    def _build_review_prompt(
        ...
        return f"""请对以下合同文本进行逐条专业审查，并输出可用于正式法律审查报告的结构化 JSON：
...
【输出要求】
1. 风险定级必须谨慎，不能仅因措辞偏强、条款不够完整就直接给出高风险。
2. 高风险仅适用于：明显违反强制性法律规定、核心金额/主体/期限存在明显错误、违约责任严重失衡、补偿标准明显可能低于法定底线、或足以高概率引发重大争议/重大损失的情形。
3. 若缺少工资标准、履约背景、主体身份、审批流程等关键事实，不能臆断结论；可明确写明“需补充事实后判断”，并优先定为中风险。
4. 不得编造法律依据，不得输出与原文无关的条款，不得夸大结论。
5. 每条审查意见必须包含：原文摘要、风险分析、风险等级、修改建议。
6. 审查结果必须能够映射为以下展示结构：
   - 📄 合同信息
   - 📊 总体评估
   - 🔍 逐条审查意见
   - 📋 审查结论汇总
7. 总体评估 summary 要像正式法务意见，明确说明核心风险集中在哪些条款；final_recommendation 必须给出“建议签署 / 建议修改后签署 / 不建议签署”之一，并可补充一句专业说明。

请严格按照 JSON 格式返回，不要输出 markdown 代码块，不要补充 JSON 之外的说明文字。"""
