...
class CustomerImportProcessor:
    ...
    def process_import_data(self, table_data, header_map):
        ...
        for index, item in enumerate(table_data, start=1):
            try:
                processed_item = self._process_single_item(item, final_header_map, index)
                if processed_item:
            ...
            with transaction.atomic():
                for item_data in valid_data:
                    try:
                        ...
                        if contact_data.get('contact_person') or contact_data.get('phone'):
                            contact = Contact(
                                customer=customer,
    ...
    def _convert_field_value(self, field, value):
        ...
        if field.field_type == 'int':
            ...
            if isinstance(value, bool):
                ...
                return 'true' if value.lower() in ['true', '1', 'yes', '是'] else 'false'
            else:
                return 'false'
        elif field.field_type == 'date':
            try:
                if isinstance(value, (int, float)):
                    return timezone.datetime.fromtimestamp(value).strftime('%Y-%m-%d')
                else:
                    return str(value)
            except Exception:
                return str(value)
            ...
            try:
                if isinstance(value, (int, float)):
                    return timezone.datetime.fromtimestamp(value).strftime('%Y-%m-%d %H:%M:%S')
                else:
                    return str(value)
            except Exception:
                return str(value)
