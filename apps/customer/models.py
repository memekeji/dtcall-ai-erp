...
class SpiderTask(models.Model):
    customer = models.ForeignKey('Customer', on_delete=models.CASCADE, related_name='spider_tasks', verbose_name='关联客户', null=True, blank=True)
    STATUS_CHOICES = (
        (1, '运行中'),
        (2, '停止'),
    )
    ...
    data_region = models.CharField(max_length=255, blank=True, null=True, verbose_name='数据地区限制')
    industry_limit = models.CharField(max_length=255, blank=True, null=True, verbose_name='行业限制')
    province = models.CharField(max_length=255, blank=True, null=True, verbose_name='省份地区')
...
class Contact(models.Model):
    customer = models.ForeignKey('Customer', on_delete=models.CASCADE, related_name='contacts', verbose_name='关联客户')
...
class CallRecord(models.Model):
    ...
    flow_id = models.CharField(blank=True, max_length=100, null=True, verbose_name='通话流水号')
...
class Customer(models.Model):
    id = models.AutoField(primary_key=True, verbose_name='ID')  # 调整为AutoField匹配原表int UNSIGNED AUTO_INCREMENT约束
    name = models.CharField(max_length=255, default='', blank=True, verbose_name='客户名称')

    customer_source = models.ForeignKey('CustomerSource', on_delete=models.SET_NULL, null=True, blank=True, db_column='source_id', verbose_name='客户来源')
    ...
    province = models.CharField(max_length=100, verbose_name='省份', default='', blank=True)
    city = models.CharField(max_length=100, verbose_name='城市', default='', blank=True)
    district = models.CharField(max_length=100, verbose_name='区/县', default='', blank=True)
    town = models.CharField(max_length=100, verbose_name='乡镇/街道', default='', blank=True)
    address = models.CharField(max_length=255, verbose_name='详细地址', default='', blank=True)
    principal = models.ForeignKey(Admin, on_delete=models.SET_NULL, null=True, blank=True, related_name='customer_principals', verbose_name='客户负责人')
    intent_status = models.PositiveIntegerField(default=0, blank=True, verbose_name='意向状态')  # 原数据库字段类型为int UNSIGNED

    admin_id = models.PositiveIntegerField(default=0, blank=True, verbose_name='录入人ID')
    belong_uid = models.PositiveIntegerField(default=0, blank=True, verbose_name='所属人ID')
    belong_did = models.PositiveIntegerField(default=0, blank=True, verbose_name='所属部门ID')
    belong_time = models.PositiveBigIntegerField(default=0, blank=True, verbose_name='获取时间')
    distribute_time = models.PositiveBigIntegerField(default=0, verbose_name='最新分配时间')
    follow_time = models.PositiveBigIntegerField(default=0, verbose_name='最新跟进时间')
    next_time = models.PositiveBigIntegerField(default=0, blank=True, verbose_name='下次跟进时间')
    discard_time = models.BigIntegerField(default=0, blank=True, verbose_name='废弃时间')
    share_ids = models.CharField(max_length=500, default='', blank=True, verbose_name='共享人员ID')
    file_ids = models.CharField(max_length=500, default='', blank=True, verbose_name='附件ids')  # 补充原表mimu_customer的file_ids字段
    ...
    tax_bank = models.CharField(max_length=255, verbose_name='开户银行', default='', blank=True)
    tax_banksn = models.CharField(max_length=255, verbose_name='银行账号', default='', blank=True)
    tax_num = models.CharField(max_length=50, verbose_name='纳税人识别号', default='', blank=True)
    tax_mobile = models.CharField(max_length=20, verbose_name='税务联系电话', default='', blank=True)
    tax_address = models.CharField(max_length=255, verbose_name='税务地址', default='', blank=True)
    is_lock = models.BooleanField(default=False, verbose_name='锁定状态')
    create_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    update_time = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    delete_time = models.PositiveBigIntegerField(default=0, verbose_name='删除时间')  # 原表为bigint UNSIGNED DEFAULT 0
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['create_time'], name='idx_customer_create_time'),
            models.Index(fields=['follow_time'], name='idx_customer_follow_time'),
            models.Index(fields=['delete_time'], name='idx_customer_delete_time'),
        ]
        constraints = [
    ...
    def get_belong_time_display(self):
        ...
        if self.belong_time and self.belong_time > 0:
            ...
            return datetime.fromtimestamp(self.belong_time).strftime('%Y-%m-%d %H:%M:%S')
        return ''
    ...
    def get_follow_time_display(self):
        ...
        if self.follow_time and self.follow_time > 0:
            from datetime import datetime
            return datetime.fromtimestamp(self.follow_time).strftime('%Y-%m-%d %H:%M:%S')
        return ''
    ...
    def get_next_time_display(self):
        ...
        if self.next_time and self.next_time > 0:
            from datetime import datetime
            return datetime.fromtimestamp(self.next_time).strftime('%Y-%m-%d %H:%M:%S')
        return ''
...
class CustomerGrade(models.Model):
    ...
    status = models.PositiveSmallIntegerField(default=1, verbose_name='状态')  # 对应原表tinyint(1) UNSIGNED
    create_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    update_time = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    delete_time = models.PositiveBigIntegerField(default=0, verbose_name='删除时间')
...
class CustomerSource(models.Model):
    ...
    sort = models.IntegerField(default=0, verbose_name='排序')
    status = models.PositiveSmallIntegerField(default=1, verbose_name='状态')  # 原表为tinyint(1) UNSIGNED DEFAULT 1
    create_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    update_time = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    delete_time = models.PositiveBigIntegerField(default=0, verbose_name='删除时间')  # 原表为bigint UNSIGNED DEFAULT 0
...
class CustomerField(models.Model):
    ...
    field_name = models.CharField(max_length=50, unique=True, verbose_name='字段标识')
...
class CustomerCustomFieldValue(models.Model):
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='custom_fields', verbose_name='客户')
    field = models.ForeignKey(CustomerField, on_delete=models.CASCADE, verbose_name='自定义字段')
...
class FollowRecord(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='follow_records', verbose_name='关联客户')
    follow_type = models.CharField(max_length=20, choices=FOLLOW_TYPE_CHOICES, default='phone', verbose_name='跟进类型')
    content = models.TextField(verbose_name='跟进内容')
    follow_user = models.ForeignKey(Admin, on_delete=models.CASCADE, verbose_name='跟进人')
...
class CustomerOrder(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='orders', verbose_name='关联客户')
    contract = models.ForeignKey('CustomerContract', on_delete=models.SET_NULL, null=True, blank=True, verbose_name='关联合同')
    ...
    amount = models.DecimalField(max_digits=15, decimal_places=2, verbose_name='订单金额')
    ...
    # 财务相关字段
    finance_status = models.CharField(max_length=20, choices=FINANCE_STATUS_CHOICES, default='pending', verbose_name='财务状态')
    invoice_request_status = models.CharField(max_length=20, choices=INVOICE_REQUEST_STATUS_CHOICES, default='none', verbose_name='开票申请状态')
    invoice_request_time = models.DateTimeField(null=True, blank=True, verbose_name='开票申请时间')
    invoice_request_user = models.ForeignKey(Admin, on_delete=models.SET_NULL, null=True, blank=True, related_name='invoice_requests', verbose_name='开票申请人')
    
    create_user = models.ForeignKey(Admin, on_delete=models.CASCADE, verbose_name='创建人')
    create_time = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    update_time = models.DateTimeField(auto_now=True, verbose_name='更新时间')
    delete_time = models.PositiveBigIntegerField(default=0, verbose_name='删除时间')
...
class CustomerContract(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='contracts', verbose_name='关联客户')
    ...
    contract_type = models.CharField(max_length=20, choices=TYPE_CHOICES, blank=True, verbose_name='合同类型')
...
class FollowRecordCustomFieldValue(models.Model):
    """跟进记录自定义字段值模型"""
    follow_record = models.ForeignKey('FollowRecord', on_delete=models.CASCADE, related_name='custom_fields', verbose_name='跟进记录')
    field = models.ForeignKey('FollowField', on_delete=models.CASCADE, verbose_name='自定义字段')
    value = models.TextField(blank=True, null=True, verbose_name='字段值')
...
class CustomerOrderCustomFieldValue(models.Model):
    """订单自定义字段值模型"""
    order = models.ForeignKey('CustomerOrder', on_delete=models.CASCADE, related_name='custom_fields', verbose_name='订单')
    field = models.ForeignKey('OrderField', on_delete=models.CASCADE, verbose_name='自定义字段')
    value = models.TextField(blank=True, null=True, verbose_name='字段值')
...
class CustomerInvoice(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='invoices', verbose_name='关联客户')
    invoice_number = models.CharField(max_length=100, unique=True, verbose_name='发票编号')
    ...
    invoice_type = models.CharField(max_length=20, choices=TYPE_CHOICES, default='ordinary', verbose_name='发票类型')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='draft', verbose_name='发票状态')
    contract_number = models.CharField(max_length=100, blank=True, verbose_name='关联合同编号')
