...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # 获取配置的流转规则天数
        no_follow_days = config_service.get_int_config(
            'customer_no_follow_days', 30)
        no_deal_days = config_service.get_int_config(
            'customer_no_deal_days', 90)

        self.stdout.write(f'开始执行客户自动流转公海任务...')
        self.stdout.write(
            f'配置规则：无跟进记录 {no_follow_days} 天流转，无成交 {no_deal_days} 天流转')
        ...
        # 计算时间阈值
        now = datetime.now()
        follow_threshold = now - \
            timedelta(days=no_follow_days) if follow_rule_enabled else None
        deal_threshold = now - \
            timedelta(days=no_deal_days) if deal_rule_enabled else None
